    ...
    while (offset < result.size()) {
        ...
        if (nextMatch.hasMatch()) {
            ...
            if (between.trimmed().isEmpty()) {
                result.insert(headingEnd, u8"<p>暂无相关内容。</p>");
                offset = headingEnd + QString::fromUtf8(u8"<p>暂无相关内容。</p>").length();
                continue;
            }
        }
...
{
    m_cancelFlag = true;
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"报告生成已请求取消"));
}
...
void ReportGenerator::generateReportAsync(const QString& filePath, const QString& format)
{
    ...
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[报告生成] 开始生成智能分析报告, 格式: %1").arg(format));
    ...
    (void)QtConcurrent::run([this, filePath, format]() {
        ...
        qt5InvokeMethod(this, [this]() {
            emit progressChanged(3, u8"加载数据...", 0);
        });
        ...
        LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[报告生成] 全量数据加载完成, 记录数: %1, 耗时: %2 ms").arg(cachedRecords.size()).arg(loadTime));

        if (m_cancelFlag) {
            qt5InvokeMethod(this, [this]() {
                m_lastError = tr(u8"报告生成已取消");
        ...
        stepStartTime = QDateTime::currentMSecsSinceEpoch();
        qt5InvokeMethod(this, [this]() {
            emit progressChanged(8, u8"单次遍历收集统计数据...", 0);
        });
        ...
        LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[报告生成] 统计数据收集完成, 耗时: %1 ms, 错误数: %2, 错误率: %3%, 成功率: %4%, 关键事件: %5")
        ...
        stepStartTime = QDateTime::currentMSecsSinceEpoch();
        qt5InvokeMethod(this, [this]() {
            emit progressChanged(30, u8"生成可视化图表...", 0);
        });
        ...
        LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[报告生成] 图表生成完成, 耗时: %1 ms").arg(chartTime));
        ...
        stepStartTime = QDateTime::currentMSecsSinceEpoch();
        qt5InvokeMethod(this, [this]() {
            emit progressChanged(45, u8"AI深度分析中（并行）...", 0);
        });
        ...
        LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[报告生成] AI分析完成, 耗时: %1 ms").arg(aiTime));
        ...
        qt5InvokeMethod(this, [this]() {
            emit progressChanged(85, u8"生成报告文件...", 0);
        });
        ...
        LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[报告生成] 报告生成完成, 总耗时: %1 秒, 数据加载: %2 秒, 统计收集: %3 秒, 图表生成: %4 秒, AI分析: %5 秒")
            .arg(seconds).arg(loadTime/1000.0).arg(statsTime/1000.0).arg(chartTime/1000.0).arg(aiTime/1000.0));

        qt5InvokeMethod(this, [this, success, filePath, seconds]() {
            emit progressChanged(100, u8"完成", seconds);
...
QString ReportGenerator::buildRecommendationPrompt(const ReportData& data) const
{
    ...
    return QString(
        u8"你是1553B总线系统优化专家。请基于以下全面分析数据，生成综合优化建议：\n\n"
        u8"数据概览：\n"
        u8"- 数据量：%1 条，时长 %2 秒\n"
        u8"- 成功率：%3%\n"
        u8"- 数据质量：%4\n\n"
        u8"主要发现：\n"
        u8"- 消息类型：BC→RT %5%，RT→BC %6%\n"
        u8"- 最活跃终端：RT%7（%8%消息）\n"
        u8"- 错误率：%9%\n"
        u8"- 总线利用率：%10%\n\n"
        u8"请生成综合建议报告（500-600字）：\n"
        u8"1. 总体评价（100字）\n"
        u8"2. 关键问题总结（150字）\n"
        u8"3. 优化建议（分短期/中期/长期，200字）\n"
        u8"4. 预期效果（100字）\n"
        u8"5. 后续行动计划（50字）\n\n"
        u8"要求：全面系统、重点突出、可操作性强。每个要点必须用<h4>标签作为小标题，然后在下方用<p>标签输出详细内容，确保每个小标题下都有具体内容，只允许使用的标签：<h4></h4>、<p></p>、<ul></ul>、<li></li>、<br/>。"
...
{
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[单次遍历] 开始收集统计数据, 记录数: %1").arg(records.size()));
    ...
    if (records.isEmpty()) {
        ...
        data.overview.dataQuality = u8"无数据";
        data.overview.activeTerminals = 0;
        data.overview.activeSubAddresses = 0;
        LOG_WARNING("ReportGenerator", QString::fromUtf8(u8"[单次遍历] 无数据，快速返回"));
        return;
    }
    ...
    for (int i = 0; i < records.size(); ++i) {
        ...
        if (messagesPerSecondVec.isEmpty()) {
            int oldSize = messagesPerSecondVec.size();
            messagesPerSecondVec.resize(second + 1);
            for (int i = oldSize; i < messagesPerSecondVec.size(); ++i)
                messagesPerSecondVec[i] = 0;
        }
    ...
    // ===== 遍历结束，打印关键诊断日志 =====
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[单次遍历] 遍历完成诊断信息: 总记录: %1, 成功数: %2, 错误数: %3, 连续错误最大: %4, 终端数: %5")
    ...
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[单次遍历] chstt采样: %1").arg(chsttSamples.join(", ")));
    // 打印各终端的错误分布
    if (!data.errorStats.errorsByTerminal.isEmpty()) {
        ...
        for (auto it = data.errorStats.errorsByTerminal.begin(); it != data.errorStats.errorsByTerminal.end(); ++it) {
            errorDist << QString::fromUtf8(u8"RT%1:%2条错误").arg(it.key()).arg(it.value());
        }
        LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[单次遍历] 错误终端分布: %1").arg(errorDist.join(", ")));
    }
    ...
    if (data.overview.duration < 0) {
        data.overview.duration = 0;
        LOG_WARNING("ReportGenerator", QString::fromUtf8(u8"[单次遍历] 警告: 计算时长为负值，已修正为0"));
    }
    ...
    if (data.overview.successRate >= 99) {
        data.overview.dataQuality = u8"优秀";
    } else if (data.overview.successRate >= 95) {
        data.overview.dataQuality = u8"良好";
    } else if (data.overview.successRate >= 90) {
        data.overview.dataQuality = u8"一般";
    } else {
        data.overview.dataQuality = u8"较差";
    }
    ...
    for (int i = 0; i < records.size(); i++) {
        if (!records[i].packetData.chstt) {
            ...
            if (consecutiveErrors >= 5 && consecutiveErrorEventAdded) {
                ...
                event.type = u8"连续错误";
                event.description = QString::fromUtf8(u8"检测到连续%1条错误记录（行%2~%3）")
                    .arg(consecutiveErrors)
                    .arg(records[errorStart].rowIndex + 1)
                    .arg(records[i - 1].rowIndex + 1);
                event.impact = u8"可能影响数据完整性";
    ...
    if (avgRate > 0 && !messagesPerSecondVec.isEmpty()) {
        for (int s = minSecond; s <= maxSecond && s < messagesPerSecondVec.size(); s++) {
            if (messagesPerSecondVec[s] > avgRate * 2.0) {
                ...
                event.type = u8"峰值负载";
                event.description = QString::fromUtf8(u8"消息速率达到%1条/秒，为平均值的%2倍")
                    .arg(messagesPerSecondVec[s]).arg(messagesPerSecondVec[s] / avgRate, 0, 'f', 1);
                event.impact = u8"可能导致通信延迟";
                data.keyEvents.append(event);
            }
        }
    }

    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[单次遍历] 统计收集完成, 总记录: %1, 成功率: %2%, 终端数: %3, 错误数: %4, 通信模式: %5, 关键事件: %6")
        .arg(data.overview.totalRecords)
...
AIAnalysisResult ReportGenerator::performAIAnalysisParallel(const ReportData& data)
{
    ...
    if (!m_modelProvider) {
        LOG_WARNING("ReportGenerator", QString::fromUtf8(u8"[AI分析] AI模型未配置，跳过AI分析"));
        result.overviewAnalysis = u8"<p style='color: #999;'>AI分析功能未启用。请在设置中配置AI模型。</p>";
    ...
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[AI分析] 开始并行AI分析，共7个分析任务"));

    // 构建所有AI提示词
    QString overviewPrompt = QString(
        u8"你是1553B总线数据分析专家。请基于以下数据生成专业的数据概览分析：\n\n"
        u8"数据信息：\n"
        u8"- 总数据量：%1 条\n"
        u8"- 数据时长：%2 秒\n"
        u8"- 成功率：%3%\n"
        ...
        u8"- 活跃终端数：%5 个\n"
        u8"- 活跃子地址数：%6 个\n\n"
        u8"请从以下角度进行分析（200-300字）：\n"
        u8"1. 数据规模评价\n"
        u8"2. 数据质量评估\n"
        u8"3. 整体通信状况\n"
        u8"4. 关键特征总结\n\n"
        u8"要求：语言专业、逻辑清晰、有说服力。每个分析要点用<h4>标签作为小标题，下方用<p>标签输出详细内容，只允许使用的标签：<h4></h4>、<p></p>、<ul></ul>、<li></li>、<br/>。"
    ).arg(data.overview.totalRecords)
     .arg(data.overview.duration, 0, 'f', 1)
     .arg(data.overview.successRate, 0, 'f', 1)
     .arg(data.overview.dataQuality)
    ...
    QString messagePrompt = QString(
        u8"你是1553B总线通信协议专家。请基于以下消息类型分布进行分析：\n\n"
        u8"消息类型分布：\n"
        u8"- BC→RT：%1 条（%2%）\n"
        u8"- RT→BC：%3 条（%4%）\n"
        u8"- RT→RT：%5 条（%6%）\n"
        ...
        u8"3. 通信模式解读\n"
        u8"4. 效率评价\n\n"
        u8"要求：深入解读、发现规律、专业分析。每个分析要点用<h4>标签作为小标题，下方用<p>标签输出详细内容，只允许使用的标签：<h4></h4>、<p></p>、<ul></ul>、<li></li>、<br/>。"
    ...
    QString terminalPrompt = QString(
        u8"你是1553B总线系统架构专家。请基于以下终端通信数据进行分析：\n\n"
        u8"终端统计（前5个最活跃）：\n");

    for (int i = 0; i < qMin(5, data.terminalStats.size()); i++) {
        const TerminalStats& ts = data.terminalStats[i];
        terminalPrompt += QString::fromUtf8(u8"- RT%1：%2 条消息（%3%），成功率 %4%\n")
            .arg(ts.terminalAddress).arg(ts.messageCount)
            .arg(ts.percent, 0, 'f', 1).arg(ts.successRate, 0, 'f', 1);
    }

    terminalPrompt += QString::fromUtf8(u8"\n活跃终端总数：%1 个\n\n")
        .arg(data.terminalStats.size());

    terminalPrompt += u8"请分析（200-300字）：\n"
        u8"1. 终端活跃度分布\n"
        u8"2. 通信负载均衡性\n"
        u8"3. 终端可靠性评估\n"
        u8"4. 优化建议\n\n"
        u8"要求：数据支撑、分析深入、建议可行。每个分析要点用<h4>标签作为小标题，下方用<p>标签输出详细内容，只允许使用的标签：<h4></h4>、<p></p>、<ul></ul>、<li></li>、<br/>。";

    QString errorPrompt = QString(
        u8"你是1553B总线故障诊断专家。请基于以下错误数据进行分析：\n\n"
        u8"错误统计：\n"
        u8"- 总错误数：%1 条\n"
        u8"- 错误率：%2%\n"
        u8"- 最大连续错误：%3 条\n\n"
    ...
    for (auto it = data.errorStats.errorsByTerminal.begin(); it != data.errorStats.errorsByTerminal.end(); ++it) {
        errorPrompt += QString::fromUtf8(u8"- RT%1：%2 条错误\n").arg(it.key()).arg(it.value());
    }

    errorPrompt += QString(u8"\n请分析（300-400字）：\n"
        u8"1. 错误严重程度评估\n"
        u8"2. 错误模式分析\n"
        u8"3. 影响评估\n"
        u8"4. 建议排查步骤\n\n"
        u8"要求：分析深入、原因具体、建议可操作。每个分析要点用<h4>标签作为小标题，下方用<p>标签输出详细内容，只允许使用的标签：<h4></h4>、<p></p>、<ul></ul>、<li></li>、<br/>。");

    QString perfPrompt = QString(
        u8"你是1553B总线性能优化专家。请基于以下性能数据进行分析：\n\n"
        u8"性能指标：\n"
        u8"- 总线利用率：%1%\n"
        u8"- 吞吐量：%2 条/秒\n"
        u8"- 平均消息间隔：%3 ms\n"
        u8"- 峰值消息速率：%4 条/秒（时间：%5）\n\n"
        u8"请进行性能分析（300-400字）：\n"
        u8"1. 性能水平评价\n"
        u8"2. 瓶颈识别\n"
        u8"3. 优化空间分析\n"
        u8"4. 具体优化建议\n\n"
        u8"要求：数据支撑、分析专业、建议可行。每个分析要点用<h4>标签作为小标题，下方用<p>标签输出详细内容，只允许使用的标签：<h4></h4>、<p></p>、<ul></ul>、<li></li>、<br/>。"
    ).arg(data.performance.busUtilization, 0, 'f', 1)
    ...
    QString riskPrompt = QString(
        u8"你是1553B总线系统安全专家。请基于以下数据进行风险评估：\n\n"
        u8"系统状态：\n"
        u8"- 数据质量：%1\n"
        u8"- 成功率：%2%\n"
        u8"- 错误率：%3%\n"
        ...
        u8"3. 潜在影响评估\n"
        u8"4. 风险缓解措施\n\n"
        u8"要求：评估客观、风险明确、措施具体。每个分析要点用<h4>标签作为小标题，下方用<p>标签输出详细内容，只允许使用的标签：<h4></h4>、<p></p>、<ul></ul>、<li></li>、<br/>。"
    ).arg(data.overview.dataQuality)
     .arg(data.overview.successRate, 0, 'f', 1)
     .arg(data.errorStats.errorRate, 0, 'f', 2)
     .arg(data.terminalStats.size())
    ...
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[AI分析] 并行AI分析全部完成"));
...
{
    if (!m_modelProvider) {
        return u8"<p style='color: #999;'>AI分析功能未启用。请在设置中配置AI模型。</p>";
    }

    QList<ChatMessage> messages;
    ChatMessage systemMsg;
    systemMsg.role = ChatMessage::System;
    systemMsg.content = u8"你是一个专业的1553B总线数据分析专家，擅长从数据中发现问题、分析原因、提出建议。"
                       u8"你的分析报告语言专业、逻辑清晰、有说服力。请用中文回答，使用HTML格式输出。"
                       u8"【HTML标签规则】"
                       u8"1. 允许使用的标签：<h4></h4>、<p></p>、<ul></ul>、<li></li>、<br/>"
                       u8"2. 严禁使用任何格式化标签：<strong>、<b>、<i>、<em>、<u>等，列表项内容直接写文字即可"
                       u8"3. 禁止使用的标签：<h1>、<h2>、<h3>、<html>、<body>、<head>、<div>、<span>"
                       u8"4. 每个开标签必须有对应的闭标签，且标签名必须完全一致"
                       u8"5. 每个小标题必须使用（1）（2）（3）等中文序号开头，格式为：<h4>（1）问题分析</h4>"
                       u8"6. 每个分析要点用<h4>标题</h4>开头，紧接着用<p>详细内容</p>或<ul><li>列表项</li></ul>"
                       u8"7. 不要在回答开头输出标题行，直接输出分析内容"
                       u8"【正确示例】"
                       u8"<h4>（1）问题分析</h4><p>经过分析发现...</p>"
                       u8"<h4>（2）优化建议</h4><ul><li>短期建议：详细说明</li><li>中期建议：详细说明</li></ul>"
                       u8"【错误示例-绝对禁止】"
                       u8"<li><strong>短期建议</strong>：说明</li>  这是错误的，不要用strong标签"
                       u8"<li>短期建议：说明</li>  这是正确的";
    ...
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[AI请求] ========== 开始AI调用 =========="));
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[AI请求] 超时设置: %1秒").arg(configTimeout));
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[AI请求] System提示词: %1").arg(systemMsg.content));
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[AI请求] User提示词长度: %1 字符").arg(prompt.length()));
    LOG_DEBUG("ReportGenerator", QString::fromUtf8(u8"[AI请求] User提示词内容:\n%1").arg(prompt));
    ...
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[AI响应] ========== AI调用返回 =========="));
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[AI响应] 耗时: %1ms, 成功: %2")
        .arg(callDuration).arg(response.success ? u8"是" : u8"否"));

    if (response.success) {
        LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[AI响应] 响应内容长度: %1 字符").arg(response.content.length()));
        QString logContent = response.content.length() > 2000 
            ? response.content.left(2000) + u8"...\n[内容过长已截断]"
            : response.content;
        LOG_DEBUG("ReportGenerator", QString::fromUtf8(u8"[AI响应] 响应内容:\n%1").arg(logContent));
        return response.content;
    } else {
        LOG_ERROR("ReportGenerator", QString::fromUtf8(u8"[AI响应] 调用失败, 错误信息: %1").arg(response.error));
        if (response.error.contains(u8"超时")) {
            return QString::fromUtf8(u8"<p style='color: #e67e22;'>AI分析超时(%1秒)，已跳过此分析项。</p>").arg(configTimeout);
        }
        return QString::fromUtf8(u8"<p style='color: #e74c3c;'>AI分析失败: %1</p>").arg(response.error);
    }
}
...
QImage ReportGenerator::generateMessageTypePieChart(const MessageStats& stats, int dpiScale)
{
    ...
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[饼图数据] BC→RT: %1条, %2%; RT→BC: %3条, %4%; RT→RT: %5条, %6%; 广播: %7条, %8%")
    ...
    painter.setFont(QFont(u8"微软雅黑", 110, QFont::Bold));
    QFontMetrics titleFm(painter.font());
    painter.drawText((scaledSize - QT5COMPAT_FONT_HADVANCE(titleFm, u8"消息类型分布")) / 2, 200, u8"消息类型分布");

    QVector<ChartSlice> slices;
    if (stats.bcToRtCount > 0) slices.append({u8"BC→RT", stats.bcToRtPercent, QColor("#3498db")});
    if (stats.rtToBcCount > 0) slices.append({u8"RT→BC", stats.rtToBcPercent, QColor("#2ecc71")});
    if (stats.rtToRtCount > 0) slices.append({u8"RT→RT", stats.rtToRtPercent, QColor("#f39c12")});
    if (stats.broadcastCount > 0) slices.append({u8"广播", stats.broadcastPercent, QColor("#e74c3c")});

    if (slices.isEmpty()) {
        slices.append({u8"无数据", 100.0, QColor("#bdc3c7")});
    }
    ...
    for (int i = 0; i < slices.size(); ++i) {
        ...
        if (showInsideLabel) {
            ...
            painter.setFont(QFont(u8"微软雅黑", 50, QFont::Bold));
            painter.setPen(Qt::white);
            QFontMetrics fmName(painter.font());
            int nameWidth = QT5COMPAT_FONT_HADVANCE(fmName, slice.label);
            painter.drawText(labelX - nameWidth/2, labelY - 15, slice.label);

            painter.setFont(QFont(u8"微软雅黑", 45, QFont::Bold));
            QString percentText = QString("%1%").arg(slice.value / total * 100.0, 0, 'f', 1);
            QFontMetrics fmPercent(painter.font());
            int percentWidth = QT5COMPAT_FONT_HADVANCE(fmPercent, percentText);
        ...
        if (horizontalDir > 0) {
            ...
            painter.drawText(textAnchorX - QT5COMPAT_FONT_HADVANCE(fmLabel, labelText), elbowY + 18, labelText);
        }
    ...
    painter.setFont(QFont(u8"微软雅黑", 60));
    painter.setPen(QColor("#7f8c8d"));
    painter.drawText(centerX - 175, centerY - 50, u8"总计");
    painter.setFont(QFont(u8"微软雅黑", 110, QFont::Bold));
    painter.setPen(QColor("#2c3e50"));
    ...
    painter.drawText(centerX - QT5COMPAT_FONT_HADVANCE(fmTotal, totalStr)/2, centerY + 100, totalStr);
    painter.setFont(QFont(u8"微软雅黑", 55));
    ...
    painter.setFont(QFont(u8"微软雅黑", 70, QFont::Bold));
    painter.setPen(QColor("#2c3e50"));
    painter.drawText(legendX, legendY - 60, u8"图例");
    ...
    for (const auto& slice : slices) {
        ...
        painter.setFont(QFont(u8"微软雅黑", 65, QFont::Bold));
        painter.setPen(QColor("#2c3e50"));
        painter.drawText(legendX + 160, legendY + 85, slice.label);

        painter.setFont(QFont(u8"微软雅黑", 65));
        painter.setPen(QColor("#7f8c8d"));
    ...
    painter.setRenderHint(QPainter::Antialiasing);
    painter.setFont(QFont(u8"微软雅黑", 50));
...
QImage ReportGenerator::generateTerminalBarChart(const QVector<TerminalStats>& stats, int dpiScale)
{
    ...
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[柱状图数据] 终端数: %1").arg(stats.size()));
    for (const auto& ts : stats) {
        LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[柱状图数据] RT%1: 消息数=%2, 占比=%3%, 成功=%4, 失败=%5, 成功率=%6%")
    ...
    // 标题：加大字号
    painter.setPen(QColor("#2c3e50"));
    painter.setFont(QFont(u8"微软雅黑", 44, QFont::Bold));
    QFontMetrics titleFm(painter.font());
    painter.drawText((imageWidth - QT5COMPAT_FONT_HADVANCE(titleFm, u8"终端消息统计")) / 2, 70, u8"终端消息统计");

    if (stats.isEmpty()) {
        painter.setFont(QFont(u8"微软雅黑", 32));
        painter.setPen(QColor("#95a5a6"));
        painter.drawText(imageWidth / 2 - 80, imageHeight / 2, u8"无数据");
        return image;
    }
    ...
    // Y轴刻度值：加大字体
    painter.setFont(QFont(u8"微软雅黑", 24));
    painter.setPen(QColor("#7f8c8d"));
    for (int i = 0; i <= 5; i++) {
        ...
        painter.drawText(chartLeft - QT5COMPAT_FONT_HADVANCE(fm, valueStr) - 22, y + 9, valueStr);
    }
    ...
    painter.rotate(-90);
    painter.setFont(QFont(u8"微软雅黑", 26));
    painter.setPen(QColor("#34495e"));
    painter.drawText(-75, 0, u8"消息数量");
    painter.restore();
    ...
    for (int i = 0; i < displayCount; i++) {
        ...
        // 柱顶数值：加大字体
        painter.setFont(QFont(u8"微软雅黑", 22, QFont::Bold));
        painter.setPen(QColor("#2c3e50"));
        QString valueText = QString::number(ts.messageCount);
        QFontMetrics fmValue(painter.font());
        painter.drawText(x + barWidth/2 - QT5COMPAT_FONT_HADVANCE(fmValue, valueText)/2, y - 12, valueText);

        // 百分比
        QString percentText = QString("(%1%)").arg(ts.percent, 0, 'f', 1);
        painter.setFont(QFont(u8"微软雅黑", 18));
        painter.setPen(QColor("#7f8c8d"));
        QFontMetrics fmPercent(painter.font());
        painter.drawText(x + barWidth/2 - QT5COMPAT_FONT_HADVANCE(fmPercent, percentText)/2, y + 30, percentText);

        painter.setFont(QFont(u8"微软雅黑", 24, QFont::Bold));
        painter.setPen(QColor("#2c3e50"));
    ...
    painter.setFont(QFont(u8"微软雅黑", 26, QFont::Bold));
    painter.setPen(QColor("#2c3e50"));
    painter.drawText(summaryBgRect.x() + 38, summaryBgRect.y() + 48, u8"统计摘要");

    int summaryY = summaryBgRect.y() + 95;
    painter.setFont(QFont(u8"微软雅黑", 22));
    painter.setPen(QColor("#7f8c8d"));
    ...
    QStringList summaryLabels = {u8"终端总数:", u8"总消息数:", u8"平均成功率:"};
...
QImage ReportGenerator::generateTimeSeriesChart(const QVector<DataRecord>& records, int dpiScale)
{
    ...
    if (records.size() < 2) {
        painter.setFont(QFont(u8"微软雅黑", 36));
        painter.setPen(QColor("#95a5a6"));
        painter.drawText(imageWidth / 2 - 100, imageHeight / 2, u8"数据不足");
        return image;
    }
    ...
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[折线图数据] 时间点数: %1, 起始秒: %2, 结束秒: %3")
    ...
    painter.setFont(QFont(u8"微软雅黑", 44, QFont::Bold));
    QFontMetrics titleFm(painter.font());
    painter.drawText((imageWidth - QT5COMPAT_FONT_HADVANCE(titleFm, u8"消息时序分布")) / 2, 70, u8"消息时序分布");
    ...
    painter.setFont(QFont(u8"微软雅黑", 26));
    painter.setPen(QColor("#34495e"));
    painter.drawText(-80, 0, u8"消息数/秒");
    painter.restore();
    ...
    for (int i = 0; i < points.size(); i++) {
        ...
        if (showLabel && values[i] > 0) {
            ...
            QFont labelFont(u8"微软雅黑", 18, QFont::Bold);
            painter.setFont(labelFont);
            QFontMetrics fm(labelFont);
            int textW = QT5COMPAT_FONT_HADVANCE(fm, valueText) + 16;
            int textH = 30;
            ...
            painter.setPen(Qt::white);
            painter.drawText(labelRect.x() + (labelRect.width() - QT5COMPAT_FONT_HADVANCE(fm, valueText))/2,
    ...
    painter.setFont(QFont(u8"微软雅黑", 26, QFont::Bold));
    painter.setPen(QColor("#2c3e50"));
    painter.drawText(statsBgRect.x() + 38, statsBgRect.y() + 48, u8"统计摘要");

    int statY = statsBgRect.y() + 98;
    QStringList statLabels = {u8"平均速率:", u8"峰值速率:", u8"时间跨度:"};
...
QImage ReportGenerator::generateErrorDistributionChart(const ErrorStats& stats, int dpiScale)
{
    ...
    painter.setPen(QColor("#2c3e50"));
    painter.setFont(QFont(u8"微软雅黑", 32, QFont::Bold));
    QFontMetrics titleFm(painter.font());
    painter.drawText((imageWidth - QT5COMPAT_FONT_HADVANCE(titleFm, u8"错误终端分布")) / 2, 60, u8"错误终端分布");

    if (stats.errorsByTerminal.isEmpty()) {
        painter.setFont(QFont(u8"微软雅黑", 24));
        painter.setPen(QColor("#27ae60"));
        painter.drawText(imageWidth / 2 - 80, imageHeight / 2, u8"✓ 无错误数据");
        return image;
    }
    ...
    for (int i = 0; i <= 5; i++) {
        ...
        QFontMetrics fm(painter.font());
        painter.drawText(chartLeft - QT5COMPAT_FONT_HADVANCE(fm, valueStr) - 20, y + 7, valueStr);
    }
    ...
    for (auto it = stats.errorsByTerminal.begin();
        ...
        painter.setFont(QFont(u8"微软雅黑", 13, QFont::Bold));
        painter.setPen(Qt::white);
        QString valueText = QString::number(it.value());
        QFontMetrics fmValue(painter.font());
        painter.drawText(badgeRect.x() + (badgeRect.width() - QT5COMPAT_FONT_HADVANCE(fmValue, valueText))/2,
                        badgeRect.y() + 19, valueText);

        painter.setFont(QFont(u8"微软雅黑", 18, QFont::Bold));
    ...
    painter.setFont(QFont(u8"微软雅黑", 18, QFont::Bold));
    painter.setPen(QColor("#c0392b"));
    painter.drawText(summaryBgRect.x() + 35, summaryBgRect.y() + 32, u8"错误摘要");

    int summaryY = summaryBgRect.y() + 65;
    QStringList errLabels = {u8"出错终端:", u8"总错误数:", u8"最大单端:"};
    ...
    for (int j = 0; j < errLabels.size(); ++j) {
        painter.setFont(QFont(u8"微软雅黑", 15));
        painter.setPen(QColor("#7f8c8d"));
        painter.drawText(summaryBgRect.x() + 18, summaryY, errLabels[j]);
        painter.setPen(QColor("#c0392b"));
        painter.setFont(QFont(u8"微软雅黑", 15, QFont::Bold));
...
QImage ReportGenerator::generateSuccessRateChart(const QVector<TerminalStats>& stats, int dpiScale)
{
    ...
    painter.setFont(QFont(u8"微软雅黑", 32, QFont::Bold));
    QFontMetrics titleFm(painter.font());
    painter.drawText((imageWidth - QT5COMPAT_FONT_HADVANCE(titleFm, u8"终端成功率统计")) / 2, 60, u8"终端成功率统计");

    if (stats.isEmpty()) {
        painter.setFont(QFont(u8"微软雅黑", 24));
        painter.setPen(QColor("#95a5a6"));
        painter.drawText(imageWidth / 2 - 60, imageHeight / 2, u8"无数据");
        return image;
    }
    ...
    painter.setFont(QFont(u8"微软雅黑", 14));
    painter.setPen(QColor("#f39c12"));
    painter.drawText(chartRight - 100, warningY - 6, u8"90% 警戒线");
    ...
    painter.setFont(QFont(u8"微软雅黑", 14));
    painter.setPen(QColor("#27ae60"));
    painter.drawText(chartRight - 100, goodY - 6, u8"95% 优秀线");
    ...
    painter.setFont(QFont(u8"微软雅黑", 18, QFont::Bold));
    painter.setPen(QColor("#2c3e50"));
    painter.drawText(summaryBgRect.x() + 35, summaryBgRect.y() + 34, u8"统计摘要");

    int sumY = summaryBgRect.y() + 70;
    QStringList rateLabels = {u8"平均成功率:", u8"优秀(≥95%):", u8"良好(90-95%):", u8"需改进(<90%):"};
    QStringList rateValues = {QString("%1%").arg(avgSuccessRate, 0, 'f', 1),
                             QString::fromUtf8(u8"%1个").arg(excellentCount),
                             QString::fromUtf8(u8"%1个").arg(warningCount),
                             QString::fromUtf8(u8"%1个").arg(criticalCount)};
...
{
    return QString(
        u8"<h2>第一章 1553B协议概述</h2>"

        u8"<h3>1.1 协议简介</h3>"
        u8"<p>MIL-STD-1553B是一种军用标准总线协议，广泛应用于航空电子系统。主要特点：</p>"
        "<ul>"
        u8"<li>数据传输速率：1 Mbps</li>"
        u8"<li>总线拓扑：主从式，支持最多31个远程终端（RT）</li>"
        u8"<li>通信方式：半双工、时分复用</li>"
        u8"<li>字长：20位（16位数据 + 3位同步头 + 1位奇偶校验）</li>"
        u8"<li>消息类型：BC→RT、RT→BC、RT→RT、广播</li>"
        "</ul>"

        u8"<h3>1.2 字格式说明</h3>"

        u8"<h4>1.2.1 命令字格式</h4>"
        u8"<p>命令字结构（20位）：</p>"
        u8"<p class='table-caption'>表1 命令字格式</p>"
        "<table class='outer-border'>"
        u8"<tr><th>同步头(3bit)</th><th>终端地址(5bit)</th><th>T/R(1bit)</th>"
        u8"<th>子地址(5bit)</th><th>数据计数(5bit)</th><th>奇偶位(1bit)</th></tr>"
        u8"<tr><td>固定模式</td><td>0-31</td><td>收/发</td>"
        u8"<td>0-31</td><td>0-31</td><td>奇校验</td></tr>"
        "</table>"

        u8"<h4>1.2.2 数据字格式</h4>"
        u8"<p>数据字结构（20位）：</p>"
        u8"<p class='table-caption'>表2 数据字格式</p>"
        "<table class='outer-border'>"
        u8"<tr><th>同步头(3bit)</th><th>数据内容(16bit)</th><th>奇偶位(1bit)</th></tr>"
        u8"<tr><td>数据模式</td><td>0x0000-0xFFFF</td><td>奇校验</td></tr>"
        "</table>"

        u8"<h4>1.2.3 状态字格式</h4>"
        u8"<p>状态字结构（20位）：</p>"
        u8"<p class='table-caption'>表3 状态字格式</p>"
        "<table class='outer-border'>"
        u8"<tr><th>同步头(3bit)</th><th>终端地址(5bit)</th><th>保留(3bit)</th>"
        u8"<th>状态标志(8bit)</th><th>奇偶位(1bit)</th></tr>"
        u8"<tr><td>状态模式</td><td>RT地址</td><td>000</td>"
        u8"<td>ME/SE/SER/RT/BS</td><td>奇校验</td></tr>"
        "</table>"

        u8"<h3>1.3 消息传输格式</h3>"

        u8"<h4>1.3.1 BC→RT传输</h4>"
        u8"<p>BC发送命令字和数据字给RT，RT接收后返回状态字确认。</p>"

        u8"<h4>1.3.2 RT→BC传输</h4>"
        u8"<p>BC发送命令字请求RT发送数据，RT返回状态字后发送数据字。</p>"

        u8"<h4>1.3.3 RT→RT传输</h4>"
        u8"<p>BC分别向接收RT和发送RT发送命令字，数据直接从发送RT传输到接收RT。</p>"

        u8"<h4>1.3.4 广播消息</h4>"
        u8"<p>BC向所有RT（地址31）发送数据，所有RT同时接收，无状态字返回。</p>"
    );
}
...
QString ReportGenerator::buildStatisticsSection(const ReportData& data)
{
    ...
    html += u8"<h2>第二章 报告概览</h2>";

    html += u8"<h3>2.1 数据基本信息</h3>";
    html += u8"<p class='table-caption'>表4 数据基本信息</p>";
    html += "<table class='outer-border'>";
    html += u8"<tr><th>项目</th><th>数值</th></tr>";
    html += QString::fromUtf8(u8"<tr><td>总数据量</td><td>%1 条</td></tr>").arg(data.overview.totalRecords);
    html += QString::fromUtf8(u8"<tr><td>数据时长</td><td>%1 秒</td></tr>").arg(data.overview.duration, 0, 'f', 2);
    html += QString::fromUtf8(u8"<tr><td>成功率</td><td>%1%</td></tr>").arg(data.overview.successRate, 0, 'f', 2);
    html += QString::fromUtf8(u8"<tr><td>数据质量</td><td>%1</td></tr>").arg(data.overview.dataQuality);
    html += QString::fromUtf8(u8"<tr><td>活跃终端数</td><td>%1 个</td></tr>").arg(data.overview.activeTerminals);
    html += QString::fromUtf8(u8"<tr><td>活跃子地址数</td><td>%1 个</td></tr>").arg(data.overview.activeSubAddresses);
    html += QString::fromUtf8(u8"<tr><td>报告生成时间</td><td>%1</td></tr>").arg(data.overview.importTime);
    html += "</table>";

    html += u8"<h3>2.2 消息类型统计</h3>";
    html += u8"<p class='table-caption'>表5 消息类型统计</p>";
    html += "<table class='outer-border'>";
    html += u8"<tr><th>消息类型</th><th>数量</th><th>占比</th></tr>";
    html += QString::fromUtf8(u8"<tr><td>BC→RT</td><td>%1 条</td><td>%2%</td></tr>")
        .arg(data.messageStats.bcToRtCount).arg(data.messageStats.bcToRtPercent, 0, 'f', 1);
    html += QString::fromUtf8(u8"<tr><td>RT→BC</td><td>%1 条</td><td>%2%</td></tr>")
        .arg(data.messageStats.rtToBcCount).arg(data.messageStats.rtToBcPercent, 0, 'f', 1);
    html += QString::fromUtf8(u8"<tr><td>RT→RT</td><td>%1 条</td><td>%2%</td></tr>")
        .arg(data.messageStats.rtToRtCount).arg(data.messageStats.rtToRtPercent, 0, 'f', 1);
    html += QString::fromUtf8(u8"<tr><td>广播</td><td>%1 条</td><td>%2%</td></tr>")
        .arg(data.messageStats.broadcastCount).arg(data.messageStats.broadcastPercent, 0, 'f', 1);
    html += "</table>";

    html += u8"<h3>2.3 终端通信统计</h3>";
    html += u8"<p class='table-caption'>表6 终端通信统计</p>";
    html += "<table class='outer-border'>";
    html += u8"<tr><th>终端地址</th><th>消息数</th><th>占比</th><th>成功数</th><th>失败数</th><th>成功率</th></tr>";
    ...
    html += u8"<h3>2.4 性能指标</h3>";
    html += u8"<p class='table-caption'>表7 性能指标</p>";
    html += "<table class='outer-border'>";
    html += u8"<tr><th>指标</th><th>数值</th></tr>";
    html += QString::fromUtf8(u8"<tr><td>总线利用率</td><td>%1%</td></tr>").arg(data.performance.busUtilization, 0, 'f', 2);
    html += QString::fromUtf8(u8"<tr><td>吞吐量</td><td>%1 条/秒</td></tr>").arg(data.performance.throughput, 0, 'f', 1);
    html += QString::fromUtf8(u8"<tr><td>平均消息间隔</td><td>%1 ms</td></tr>").arg(data.performance.avgInterval, 0, 'f', 2);
    html += QString::fromUtf8(u8"<tr><td>峰值消息速率</td><td>%1 条/秒</td></tr>").arg(data.performance.peakMessageRate);
    html += QString::fromUtf8(u8"<tr><td>峰值时间</td><td>%1</td></tr>").arg(data.performance.peakTime);
    html += "</table>";

    html += u8"<h3>2.5 错误统计</h3>";
    html += u8"<p class='table-caption'>表8 错误统计</p>";
    html += "<table class='outer-border'>";
    html += u8"<tr><th>项目</th><th>数值</th></tr>";
    html += QString::fromUtf8(u8"<tr><td>总错误数</td><td>%1 条</td></tr>").arg(data.errorStats.totalErrors);
    html += QString::fromUtf8(u8"<tr><td>错误率</td><td>%1%</td></tr>").arg(data.errorStats.errorRate, 0, 'f', 2);
    html += QString::fromUtf8(u8"<tr><td>最大连续错误</td><td>%1 条</td></tr>").arg(data.errorStats.maxConsecutiveErrors);
    html += "</table>";

    if (!data.patterns.isEmpty()) {
        html += u8"<h3>2.6 检测到的通信模式</h3>";
        html += u8"<p class='table-caption'>表9 检测到的通信模式</p>";
        html += "<table class='outer-border'>";
        html += u8"<tr><th>终端</th><th>子地址</th><th>周期(ms)</th><th>消息数</th><th>稳定性</th></tr>";
    ...
    if (!data.keyEvents.isEmpty()) {
        html += u8"<h3>2.7 关键事件</h3>";
        html += u8"<p class='table-caption'>表10 关键事件</p>";
        html += "<table class='outer-border'>";
        html += u8"<tr><th>时间</th><th>类型</th><th>描述</th><th>影响</th></tr>";
...
{
    QString html;
    html += u8"<h2>第三章 可视化图表</h2>";

    html += "<div class='chart-container'>";
    html += u8"<h3>3.1 消息类型分布</h3>";
    html += "<img src='chart_messageTypePie'>";
    html += u8"<p class='figure-caption'>图1 消息类型分布饼图</p>";
    html += "</div>";

    html += "<div class='chart-container'>";
    html += u8"<h3>3.2 终端消息统计</h3>";
    html += "<img src='chart_terminalBar'>";
    html += u8"<p class='figure-caption'>图2 终端消息统计柱状图</p>";
    html += "</div>";

    html += "<div class='chart-container'>";
    html += u8"<h3>3.3 消息时序分布</h3>";
    html += "<img src='chart_timeSeries'>";
    html += u8"<p class='figure-caption'>图3 消息时序分布折线图</p>";
    html += "</div>";

    html += "<div class='chart-container'>";
    html += u8"<h3>3.4 错误终端分布</h3>";
    html += "<img src='chart_errorDistribution'>";
    html += u8"<p class='figure-caption'>图4 错误终端分布图</p>";
    html += "</div>";

    html += "<div class='chart-container'>";
    html += u8"<h3>3.5 终端成功率统计</h3>";
    html += "<img src='chart_successRate'>";
    html += u8"<p class='figure-caption'>图5 终端成功率统计图</p>";
    html += "</div>";
...
QString ReportGenerator::buildReportHtml(const ReportData& data, const StyleConfig& style)
{
    ...
    if (style.isPdf) {
        ...
        html += u8"<title>1553B总线数据分析报告</title>";
        html += "<style>";
        html += "@page { size: A4; margin: 2.5cm 2.5cm 2.5cm 3cm; }";
    }

    html += QString(u8"body { font-family: 'SimSun', '宋体', serif; font-size: %1; color: #000000; "
    ...
    html += QString(u8"h1 { font-family: 'SimHei', '黑体', sans-serif; font-size: %1; font-weight: bold; "
    ...
    html += QString(u8"h2 { font-family: 'SimHei', '黑体', sans-serif; font-size: %1; font-weight: bold; "
    ...
    html += QString(u8"h3 { font-family: 'SimSun', '宋体', serif; font-size: %1; font-weight: bold; "
            "text-align: left; color: #000000; line-height: 24pt; margin-top: 18pt; margin-bottom: 6pt; }")
            .arg(style.h3FontSize);

    html += QString(u8"h4 { font-family: 'SimSun', '宋体', serif; font-size: %1; font-weight: %2; "
            "text-align: left; color: #000000; line-height: 24pt; margin-top: 12pt; margin-bottom: 3pt; }")
            .arg(style.h4FontSize, style.h4FontWeight);

    html += QString(u8"p { font-family: 'SimSun', '宋体', serif; font-size: %1; color: #000000; "
    ...
    html += QString(u8"ul, ol { font-family: 'SimSun', '宋体', serif; font-size: %1; color: #000000; "
    ...
    html += QString(u8"th { font-family: 'SimSun', '宋体', serif; font-size: %1; font-weight: bold; "
    ...
    html += QString(u8"td { font-family: 'SimSun', '宋体', serif; font-size: %1; color: #000000; "
    ...
    html += QString(u8".table-caption { font-family: 'SimSun', '宋体', serif; font-size: %1; font-weight: bold; "
            "color: #000000; text-align: center; line-height: 14pt; margin-top: 12pt; margin-bottom: 6pt; }")
            .arg(style.captionFontSize);

    html += QString(u8".figure-caption { font-family: 'SimSun', '宋体', serif; font-size: %1; font-weight: bold; "
    ...
    if (style.isPdf) {
        ...
        html += u8"<h1>1553B总线数据分析报告</h1>";
    } else {
        html += "<div class='header-info'></div>";
        html += u8"<h1 class='title'>1553B总线数据分析报告</h1>";
    }
    ...
    html += QString::fromUtf8(u8"<p class='no-indent'>本报告由1553B数据智能分析工具自动生成，报告生成时间：%1</p>").arg(data.overview.importTime);
    html += "</div>";
...
bool ReportGenerator::exportToPdf(const QString& filePath, const ReportData& data)
{
    ...
    QT5COMPAT_PRINTER_MARGINS(printer, 25, 25, 25, 25, QPrinter::Millimeter);
    ...
    qreal leftMm = 25, topMm = 25, rightMm = 25, bottomMm = 25;
    printer.getPageMargins(&leftMm, &topMm, &rightMm, &bottomMm, QPrinter::Millimeter);
    QMarginsF marginsMm(leftMm, topMm, rightMm, bottomMm);
#endif
    ...
    // 饼图单独使用90%宽度（缩小10%）
    // 注意：使用精确匹配（包含>结尾），避免误替换用户数据中的相似文本
    ...
    // 其他图表使用标准宽度，精确匹配完整的img标签
    html.replace("<img src='chart_terminalBar'>",
                 QString("<br><img width='%1' src='chart_terminalBar'>").arg(chartDisplayWidthPt));
    html.replace("<img src='chart_timeSeries'>",
                 QString("<br><img width='%1' src='chart_timeSeries'>").arg(chartDisplayWidthPt));
    html.replace("<img src='chart_errorDistribution'>",
                 QString("<br><img width='%1' src='chart_errorDistribution'>").arg(chartDisplayWidthPt));
    html.replace("<img src='chart_successRate'>",
                 QString("<br><img width='%1' src='chart_successRate'>").arg(chartDisplayWidthPt));
...
bool ReportGenerator::exportToDocx(const QString& filePath, const ReportData& data)
{
    ...
    entries.append(ZipEntry{"docProps/core.xml",
        ...
        u8" <dc:title>1553B智能分析报告</dc:title>\n"
    ...
    if (!outFile.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        m_lastError = QString::fromUtf8(u8"无法创建DOCX文件: %1").arg(filePath);
        return false;
    }
    ...
    LOG_INFO("ReportGenerator", QString::fromUtf8(u8"[DOCX导出] 报告已导出至: %1, 文件数: %2").arg(filePath).arg(entries.size()));
    return true;
}
...
bool ReportGenerator::exportToHtml(const QString& filePath, const ReportData& data)
{
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        m_lastError = QString::fromUtf8(u8"无法打开文件: %1").arg(filePath);
        return false;
    }
