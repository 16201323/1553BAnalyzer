...
void PaginationWidget::setupUI()
{
    ...
    // 中间：页码导航
    m_prevBtn = new QPushButton(tr(u8"上一页"));
    ...
    // 页码显示和跳转
    m_pageLabel = new QLabel(tr(u8"第"));
    ...
    m_totalPagesLabel = new QLabel(tr(u8"页 / 共 0 页"));
    mainLayout->addWidget(m_totalPagesLabel);
    
    m_nextBtn = new QPushButton(tr(u8"下一页"));
    ...
    // 右侧：每页条数设置
    m_pageSizeLabel = new QLabel(tr(u8"每页显示:"));
    ...
    QLabel* unitLabel = new QLabel(tr(u8"条"));
...
void PaginationWidget::updatePagination(int currentPage, int totalPages, int filteredCount, int totalCount)
{
    ...
    m_totalPagesLabel->setText(tr(u8"页 / 共 %1 页").arg(totalPages));
...
{
    // 更新数据总数
    m_totalCountLabel->setText(tr(u8"数据总数: %1 条").arg(m_totalCount));
    
    // 更新筛选后总数
    if (m_filteredCount != m_totalCount) {
        m_filteredCountLabel->setText(tr(u8"筛选后: %1 条").arg(m_filteredCount));
    ...
    if (m_filteredCount > 0 && m_totalPages > 0) {
        ...
        m_rangeLabel->setText(tr(u8"显示: %1-%2 条").arg(start).arg(end));
    } else {
        m_rangeLabel->setText(tr(u8"显示: 0-0 条"));
    }
}
