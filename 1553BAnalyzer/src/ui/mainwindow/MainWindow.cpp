...
{
    qRegisterMetaType<QVector<PacketParser::SMbiMonPacketMsg>>("QVector<PacketParser::SMbiMonPacketMsg>");
    ...
    connect(ConfigManager::instance(), &ConfigManager::configChanged, this, [this]() {
        LOG_INFO("MainWindow", QString::fromUtf8(u8"配置已变更，即时生效"));
        loadSettings();
    });
    
    setAcceptDrops(true);
    setWindowTitle(tr(u8"1553B数据智能分析工具"));
    resize(1280, 800);
...
void MainWindow::setupUI()
{
    ...
    m_centralTab->addTab(tablePage, tr(u8"数据表格"));
    
    m_ganttView = new GanttView(this);
    m_centralTab->addTab(m_ganttView, tr(u8"甘特图"));
    
    m_chartWidget = new ChartWidget(this);
    m_centralTab->addTab(m_chartWidget, tr(u8"统计图表"));
...
void MainWindow::setupMenuBar()
{
    ...
    QMenu* fileMenu = menuBar->addMenu(tr(u8"文件(&F)"));
    fileMenu->addAction(tr(u8"打开文件(&O)"), this, &MainWindow::onOpenFile, QKeySequence::Open);
    fileMenu->addSeparator();
    fileMenu->addAction(tr(u8"导出数据(&E)"), this, &MainWindow::onSaveExport, QKeySequence::Save);
    
    QMenu* exportMenu = fileMenu->addMenu(tr(u8"导出图表"));
    exportMenu->addAction(tr(u8"导出甘特图(PNG)"), this, &MainWindow::onExportGanttPng);
    exportMenu->addAction(tr(u8"导出甘特图(PDF)"), this, &MainWindow::onExportGanttPdf);
    
    fileMenu->addSeparator();
    fileMenu->addAction(tr(u8"生成智能分析报告(&R)"), this, &MainWindow::onGenerateReport, QKeySequence(Qt::CTRL + Qt::Key_R));
    fileMenu->addAction(tr(u8"生成测试数据(&T)"), this, &MainWindow::onGenerateTestData, QKeySequence(Qt::CTRL + Qt::Key_T));
    fileMenu->addAction(tr(u8"查看16进制(&H)"), this, &MainWindow::onViewHex, QKeySequence(Qt::CTRL + Qt::Key_H));
    
    fileMenu->addSeparator();
    fileMenu->addAction(tr(u8"退出(&X)"), this, &QWidget::close, QKeySequence::Quit);
    
    QMenu* editMenu = menuBar->addMenu(tr(u8"编辑(&E)"));
    editMenu->addAction(tr(u8"设置(&S)"), this, &MainWindow::onSettings, QKeySequence::Preferences);
    
    QMenu* viewMenu = menuBar->addMenu(tr(u8"视图(&V)"));
    viewMenu->addAction(tr(u8"文件列表"), [this]() {
        if (m_fileListPanel) m_fileListPanel->show();
    });
    viewMenu->addAction(tr(u8"AI查询面板"), [this]() {
        if (m_aiQueryPanel) m_aiQueryPanel->show();
    });
    
    QMenu* helpMenu = menuBar->addMenu(tr(u8"帮助(&H)"));
    helpMenu->addAction(tr(u8"关于(&A)"), this, &MainWindow::onAbout);
}
...
{
    QToolBar* toolBar = addToolBar(tr(u8"主工具栏"));
    toolBar->setObjectName("MainToolBar");
    toolBar->setMovable(false);
    toolBar->setToolButtonStyle(Qt::ToolButtonTextOnly);
    
    // 添加左侧按钮
    toolBar->addAction(tr(u8"打开"), this, &MainWindow::onOpenFile);
    toolBar->addAction(tr(u8"导出"), this, &MainWindow::onSaveExport);
    toolBar->addAction(tr(u8"设置"), this, &MainWindow::onSettings);
    toolBar->addAction(tr(u8"日志"), this, &MainWindow::onOpenLog);
    
    // 添加数据范围选择器（在日志按钮右边）
    QLabel* scopeLabel = new QLabel(tr(u8" 数据范围: "), this);
    scopeLabel->setObjectName("DataScopeLabel");
    toolBar->addWidget(scopeLabel);
    
    m_dataScopeCombo = new QComboBox(this);
    m_dataScopeCombo->setObjectName("DataScopeCombo");
    m_dataScopeCombo->addItem(tr(u8"文件所有数据"), static_cast<int>(DataScope::AllData));
    m_dataScopeCombo->addItem(tr(u8"当前筛选数据"), static_cast<int>(DataScope::FilteredData));
    m_dataScopeCombo->addItem(tr(u8"当前页数据"), static_cast<int>(DataScope::CurrentPage));
    m_dataScopeCombo->setToolTip(tr(u8"选择甘特图和统计图表的数据范围"));
...
void MainWindow::setupStatusBar()
{
    ...
    m_statusLabel = new QLabel(tr(u8"就绪"));
    m_countLabel = new QLabel(tr(u8"数据: 0 条"));
...
void MainWindow::setupConnections()
{
    ...
    /* 连接甘特图和图表更新完成信号，用于关闭加载提示对话框 */
    connect(m_ganttView, &GanttView::updateFinished, this, &MainWindow::onGanttUpdateFinished);
    connect(m_chartWidget, &ChartWidget::updateFinished, this, &MainWindow::onChartUpdateFinished);
    ...
    connect(m_fileListPanel, &FileListPanel::fileRemoved, this, [this](const QString& filePath, int fileId) {
        LOG_INFO("MainWindow", QString::fromUtf8(u8"文件从列表移除: %1, fileId: %2").arg(filePath).arg(fileId));
        if (fileId > 0 && DatabaseManager::instance()->isInitialized()) {
            QProgressDialog progress(tr(u8"正在删除文件数据..."), tr(u8"取消"), 0, 1, this);
            progress.setWindowModality(Qt::WindowModal);
            progress.setMinimumDuration(0);
            progress.setAutoClose(true);
            progress.setAutoReset(true);
            progress.setLabelText(tr(u8"正在删除文件: %1").arg(QFileInfo(filePath).fileName()));
            progress.setValue(0);
            QCoreApplication::processEvents();
            
            bool ok = DatabaseManager::instance()->deleteFileData(fileId);
            LOG_INFO("MainWindow", QString::fromUtf8(u8"数据库删除文件数据: fileId=%1, 结果=%2").arg(fileId).arg(ok));
    ...
    connect(m_fileListPanel, &FileListPanel::filesCleared, this, [this](const QVector<int>& fileIds) {
        LOG_INFO("MainWindow", QString::fromUtf8(u8"文件列表清空, 共%1个数据库文件需要删除").arg(fileIds.size()));
        if (DatabaseManager::instance()->isInitialized() && !fileIds.isEmpty()) {
            QProgressDialog progress(tr(u8"正在删除文件数据..."), tr(u8"取消"), 0, fileIds.size(), this);
            progress.setWindowModality(Qt::WindowModal);
            progress.setMinimumDuration(0);
            progress.setAutoClose(true);
            progress.setAutoReset(true);
            progress.setLabelText(tr(u8"正在删除数据库数据 (0/%1)").arg(fileIds.size()));
            progress.setValue(0);
            QCoreApplication::processEvents();
            
            for (int i = 0; i < fileIds.size(); ++i) {
                if (progress.wasCanceled()) {
                    LOG_INFO("MainWindow", QString::fromUtf8(u8"用户取消删除数据库数据"));
                    break;
                }
                progress.setLabelText(tr(u8"正在删除数据库数据 (%1/%2)").arg(i + 1).arg(fileIds.size()));
                progress.setValue(i);
                QCoreApplication::processEvents();
                
                bool ok = DatabaseManager::instance()->deleteFileData(fileIds[i]);
                LOG_INFO("MainWindow", QString::fromUtf8(u8"数据库删除文件数据: fileId=%1, 结果=%2").arg(fileIds[i]).arg(ok));
            }
    ...
    connect(m_aiQueryPanel, &AIQueryPanel::modelChanged, this, [this](const QString& modelId) {
        ...
        m_statusLabel->setText(tr(u8"已切换模型: %1").arg(modelId));
    });
    connect(m_aiQueryPanel, &AIQueryPanel::cancelRequested, this, [this]() {
        m_statusLabel->setText(tr(u8"查询已取消"));
        LOG_INFO("MainWindow", QString::fromUtf8(u8"AI查询已取消"));
    });
    connect(m_aiQueryPanel, &AIQueryPanel::resetDataRequested, this, [this]() {
        ...
        m_progressDialog->setWindowTitle(tr(u8"正在恢复数据"));
        m_progressDialog->setStatusText(tr(u8"正在清除筛选条件..."));
        ...
        m_progressDialog->setStatusText(tr(u8"正在刷新数据..."));
        QCoreApplication::processEvents();
        ...
        QString elapsedStr = elapsed < 1000 ? tr(u8"%1 毫秒").arg(elapsed) :
                             tr(u8"%1 秒").arg(QString::number(elapsed / 1000.0, 'f', 2));
        m_progressDialog->setStatusText(tr(u8"恢复完成！耗时 %1").arg(elapsedStr));
        ...
        m_statusLabel->setText(tr(u8"已恢复显示全部数据"));
        LOG_INFO("MainWindow", QString::fromUtf8(u8"数据已重置，显示全部数据"));
    });
    ...
    connect(m_toolExecutor, &AIToolExecutor::timeIntervalAnalysisRequested, this, &MainWindow::onAITimeIntervalAnalysisRequested);
...
void MainWindow::loadSettings()
{
    ...
    if (providers.isEmpty()) {
        LOG_WARNING("MainWindow", QString::fromUtf8(u8"模型提供商列表为空，无法初始化模型选择器"));
    } else {
...
void MainWindow::loadDatabaseFileList()
{
    ...
    LOG_INFO("MainWindow", QString::fromUtf8(u8"从数据库加载了 %1 个文件记录").arg(files.size()));
}
...
bool MainWindow::loadFileFromDatabase(int fileId)
{
    ...
    if (fileInfo.isEmpty()) {
        QMessageBox::warning(this, tr(u8"加载失败"), tr(u8"数据库中未找到该文件记录"));
        return false;
    }
    ...
    m_progressDialog->setWindowTitle(tr(u8"正在加载数据"));
    m_progressDialog->setStatusText(tr(u8"正在从数据库加载数据..."));
    m_progressDialog->setProgress(0);
    m_progressDialog->startTimer();
    m_progressDialog->show();
    ...
    m_progressDialog->setStatusText(tr(u8"正在切换数据源..."));
    QCoreApplication::processEvents();
    ...
    m_progressDialog->setStatusText(tr(u8"正在查询数据记录..."));
    QCoreApplication::processEvents();
    ...
    if (!ok) {
        ...
        m_progressDialog->hide();
        m_progressDialog->reset();
        QMessageBox::warning(this, tr(u8"加载失败"), tr(u8"无法从数据库加载文件数据"));
        return false;
    }
    
    m_progressDialog->setProgress(60);
    m_progressDialog->setStatusText(tr(u8"正在加载数据页面..."));
    QCoreApplication::processEvents();
    
    m_dataStore->setPageSize(m_paginationWidget->pageSize());
    
    m_progressDialog->setProgress(80);
    m_progressDialog->setStatusText(tr(u8"正在更新界面..."));
    QCoreApplication::processEvents();
    ...
    m_statusLabel->setText(tr(u8"已从数据库加载: %1").arg(fileInfo["file_name"].toString()));
    m_countLabel->setText(tr(u8"数据: %1 条").arg(m_dataStore->filteredCount()));
    updateWindowTitle();
    
    m_fileListPanel->setActiveFile(filePath);
    
    m_progressDialog->setProgress(100);
    m_progressDialog->setStatusText(tr(u8"加载完成！"));
    QCoreApplication::processEvents();
    ...
    LOG_INFO("MainWindow", QString::fromUtf8(u8"从数据库加载文件: fileId=%1, path=%2, count=%3")
...
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"开始加载文件: %1").arg(filePath));
    ...
    if (DatabaseManager::instance()->isInitialized()) {
        ...
        if (fileId > 0) {
            LOG_INFO("MainWindow", QString::fromUtf8(u8"文件已在数据库中(fileId=%1)").arg(fileId));
            
            QMessageBox::StandardButton reply = QMessageBox::question(this,
                tr(u8"文件已导入"),
                tr(u8"该文件已导入数据库，是否加载显示数据？"),
    ...
    m_progressDialog->setWindowTitle(tr(u8"正在加载文件"));
    m_progressDialog->setStatusText(tr(u8"正在解析文件..."));
    m_progressDialog->setProgress(0);
    m_progressDialog->startTimer();
    m_progressDialog->show();
    ...
    LOG_INFO("MainWindow", QString::fromUtf8(u8"文件导入开始时间: %1 ms (相对时间)").arg(startMs));
    
    ParserConfig config = ConfigManager::instance()->getParserConfig();
    LOG_DEBUG("MainWindow", QString::fromUtf8(u8"解析配置: 字节序=%1, Header1=0x%2, Header2=0x%3, DataHeader=0x%4, 错误容差=%5")
...
void MainWindow::onSaveExport()
{
    if (m_dataStore->totalCount() == 0) {
        QMessageBox::warning(this, tr(u8"导出失败"), tr(u8"没有数据可导出，请先导入数据文件。"));
        return;
    }
    
    // 数据范围选择对话框
    QMessageBox msgBox(this);
    msgBox.setWindowTitle(tr(u8"选择导出范围"));
    msgBox.setText(tr(u8"请选择要导出的数据范围："));
    msgBox.setStandardButtons(QMessageBox::Yes | QMessageBox::No | QMessageBox::Cancel);
    msgBox.setButtonText(QMessageBox::Yes, tr(u8"文件所有数据"));
    msgBox.setButtonText(QMessageBox::No, tr(u8"当前筛选数据"));
    msgBox.setButtonText(QMessageBox::Cancel, tr(u8"当前页数据"));
    msgBox.setDefaultButton(QMessageBox::No);
    
    DataScope exportScope = DataScope::FilteredData; // 默认导出筛选后的数据
    
    int result = msgBox.exec();
    if (result == QMessageBox::Yes) {
        exportScope = DataScope::AllData;
    } else if (result == QMessageBox::Cancel) {
        exportScope = DataScope::CurrentPage;
    }
    
    // 生成默认文件名
    QString defaultFileName = QDateTime::currentDateTime().toString("yyyyMMdd_HHmmss") + "_1553B_data";
    
    QString filePath = QFileDialog::getSaveFileName(
        this,
        tr(u8"导出数据"),
        defaultFileName,
        tr(u8"CSV文件 (*.csv);;PDF文件 (*.pdf)")
    );
    ...
    QProgressDialog progressDialog(tr(u8"正在导出数据..."), tr(u8"取消"), 0, 100, this);
    progressDialog.setWindowTitle(tr(u8"导出数据"));
    ...
    connect(&progressDialog, &QProgressDialog::canceled, this, [this]() {
        m_statusLabel->setText(tr(u8"导出已取消"));
    });
    
    m_statusLabel->setText(tr(u8"导出中..."));
    
    bool success = false;
    if (filePath.endsWith(".csv", Qt::CaseInsensitive)) {
        success = m_exportService->exportToCsv(filePath, m_dataStore, exportScope);
    } else if (filePath.endsWith(".pdf", Qt::CaseInsensitive)) {
        success = m_exportService->exportToPdf(filePath, m_dataStore, exportScope);
    }
    ...
    if (success) {
        m_statusLabel->setText(tr(u8"导出成功"));
        QMessageBox msgBox(this);
        msgBox.setWindowTitle(tr(u8"导出成功"));
        msgBox.setText(tr(u8"数据已导出到: %1").arg(filePath));
        ...
        m_statusLabel->setText(tr(u8"导出失败"));
        QMessageBox::warning(this, tr(u8"导出失败"), m_exportService->lastError());
    }
}
...
void MainWindow::onExportGanttPng()
{
    ...
    if (!filePath.isEmpty()) {
        if (m_ganttView->exportToImage(filePath)) {
            QMessageBox msgBox(this);
            msgBox.setWindowTitle(tr(u8"导出成功"));
            msgBox.setText(tr(u8"甘特图已保存到: %1").arg(filePath));
            msgBox.setStandardButtons(QMessageBox::Open | QMessageBox::Ok);
            msgBox.setDefaultButton(QMessageBox::Open);
            ...
            QMessageBox::warning(this, tr(u8"导出失败"), tr(u8"无法导出甘特图，请确保已加载数据。"));
        }
    }
}
...
void MainWindow::onOpenLog()
{
    ...
    if (logPath.isEmpty()) {
        QMessageBox::information(this, tr(u8"打开日志"), tr(u8"日志文件尚未创建。"));
        return;
    }
    
    QFileInfo fileInfo(logPath);
    if (!fileInfo.exists()) {
        QMessageBox::warning(this, tr(u8"打开日志"), tr(u8"日志文件不存在:\n%1").arg(logPath));
        return;
    }
    
    LOG_INFO("MainWindow", QString::fromUtf8(u8"打开日志文件: %1").arg(logPath));
    
    QUrl url = QUrl::fromLocalFile(logPath);
    if (!QDesktopServices::openUrl(url)) {
        QMessageBox::warning(this, tr(u8"打开日志"), tr(u8"无法打开日志文件:\n%1").arg(logPath));
    }
}
...
void MainWindow::onParseProgress(int current, int total)
{
    if (m_progressDialog) {
        ...
        m_progressDialog->setStatusText(tr(u8"正在解析文件... %1%").arg(current * 100 / total));
    }
}
...
void MainWindow::onParseFinished(bool success, int count)
{
    if (m_loadCanceled.load()) {
        LOG_INFO("MainWindow", QString::fromUtf8(u8"加载已取消，忽略解析完成回调"));
        return;
    }
    ...
    if (success) {
        ...
        if (m_progressDialog) {
            m_progressDialog->setProgress(60);
            m_progressDialog->setStatusText(tr(u8"解析完成，正在准备数据..."));
        }
        
        QtConcurrent::run([this]() {
                qt5InvokeMethodBlocking(this, [this]() {
                    if (m_progressDialog) {
                        m_progressDialog->setProgress(62);
                        m_progressDialog->setStatusText(tr(u8"正在获取解析数据..."));
                    }
                });
            
            QVector<PacketParser::SMbiMonPacketMsg> parsedData;
            QMetaObject::invokeMethod(m_parser, "getParsedData", Qt::BlockingQueuedConnection,
                Q_RETURN_ARG(QVector<PacketParser::SMbiMonPacketMsg>, parsedData));
            int actualCount = parsedData.size();
            
            LOG_DEBUG("MainWindow", QString::fromUtf8(u8"实际解析的消息数: %1").arg(actualCount));
            
            if (m_loadCanceled.load()) {
                LOG_INFO("MainWindow", QString::fromUtf8(u8"加载已取消，停止数据处理"));
                return;
            }
            ...
            if (actualCount == 0) {
                qt5InvokeMethodBlocking(this, [this]() {
                    ...
                    m_statusLabel->setText(tr(u8"解析完成，但未找到有效数据"));
                    QMessageBox::warning(this, tr(u8"解析结果"), 
                        tr(u8"文件解析成功，但未找到任何有效的1553B数据。\n\n"
                           u8"可能的原因:\n"
                           u8"1. 文件格式与配置不匹配\n"
                           u8"2. Header1/Header2/DataHeader配置错误\n"
                           u8"3. 字节序配置错误\n\n"
                           u8"请检查设置中的解析器配置。"));
                });
                return;
            ...
            qt5InvokeMethodBlocking(this, [this]() {
                if (m_progressDialog) {
                    ...
                    m_progressDialog->setStatusText(tr(u8"正在构建索引..."));
                }
            });
            ...
            QVector<DataRecord> index = DataStore::buildIndex(parsedData, 
                [this](int current, int total) {
                    ...
                    qt5InvokeMethodBlocking(this, [this, buildPercent, current, total]() {
                        if (m_progressDialog) {
                            m_progressDialog->setProgress(buildPercent);
                            m_progressDialog->setStatusText(tr(u8"正在构建索引... %1/%2").arg(current).arg(total));
                        }
                    });
                }
            );
            ...
            qt5InvokeMethodBlocking(this, [this]() {
                if (m_progressDialog) {
                    ...
                    m_progressDialog->setStatusText(tr(u8"正在加载数据..."));
                }
            });
            ...
            LOG_INFO("MainWindow", QString::fromUtf8(u8"表格加载开始时间: %1 ms (相对时间)").arg(elapsedMs));
            
            qt5InvokeMethodBlocking(this, [this]() {
                if (m_progressDialog) {
                    m_progressDialog->setProgress(87);
                    m_progressDialog->setStatusText(tr(u8"正在加载数据到内存..."));
                }
            });
            ...
            if (estimatedRecords > threshold) {
                LOG_INFO("MainWindow", QString::fromUtf8(u8"数据量 %1 超过阈值，使用数据库模式").arg(estimatedRecords));
                
                qt5InvokeMethodBlocking(this, [this]() {
                    if (m_progressDialog) {
                        m_progressDialog->setStatusText(tr(u8"正在导入数据到数据库..."));
                    }
                    m_dataStore->setStorageMode(StorageMode::Database);
                });
                ...
                int fileId = m_dataStore->importFromData(parsedData, m_currentFile,
                    [this](int current, int total, const QString& status) {
                        qt5InvokeMethodBlocking(this, [this, current, total, status]() {
                            if (m_progressDialog) {
                ...
                if (fileId <= 0) {
                    if (m_loadCanceled.load()) {
                        LOG_INFO("MainWindow", QString::fromUtf8(u8"导入已取消，不回退内存模式"));
                        return;
                    }
                    LOG_ERROR("MainWindow", QString::fromUtf8(u8"数据库导入失败，回退到内存模式"));
                    qt5InvokeMethodBlocking(this, [this, parsedData, index]() {
                        m_dataStore->setStorageMode(StorageMode::Memory);
                        m_dataStore->setIndexedData(parsedData, index);
                    });
                } else {
                    qt5InvokeMethodBlocking(this, [this, fileId]() {
                        m_fileListPanel->updateFileId(m_currentFile, fileId);
                        m_fileListPanel->setActiveFile(m_currentFile);
                    });
                }
            } else {
                LOG_INFO("MainWindow", QString::fromUtf8(u8"数据量 %1 未超过阈值，使用内存模式").arg(estimatedRecords));
                qt5InvokeMethodBlocking(this, [this, parsedData, index]() {
                    m_dataStore->setIndexedData(parsedData, index);
                    m_fileListPanel->setActiveFile(m_currentFile);
                });
            ...
            qt5InvokeMethodBlocking(this, [this, actualCount]() {
                if (m_loadCanceled.load()) {
                    LOG_INFO("MainWindow", QString::fromUtf8(u8"加载已取消，跳过UI更新"));
                    return;
                }
                ...
                qint64 elapsedMs = m_elapsedTimer.elapsed();
                LOG_INFO("MainWindow", QString::fromUtf8(u8"表格加载完毕时间: %1 ms (相对时间)").arg(elapsedMs));
                ...
                LOG_INFO("MainWindow", QString::fromUtf8(u8"解析完成: count=%1, 总耗时=%2ms (%3)")
                         .arg(actualCount).arg(elapsedMs).arg(timeStr));
                
                // 更新状态栏
                m_statusLabel->setText(tr(u8"解析完成"));
                m_countLabel->setText(tr(u8"数据: %1 条").arg(m_dataStore->filteredCount()));
                updateWindowTitle();
                ...
                if (m_progressDialog) {
                    ...
                    m_progressDialog->setStatusText(tr(u8"正在更新分页控件..."));
                }
                ...
                LOG_INFO("MainWindow", QString::fromUtf8(u8"数据加载完成，共 %1 条消息，%2 条数据记录")
                         .arg(actualCount)
                ...
                QTimer::singleShot(100, this, [this, actualCount, timeStr]() {
                    ...
                    // 显示完成消息
                    QMessageBox::information(this, tr(u8"解析完成"), 
                        tr(u8"文件解析成功！\n\n"
                           u8"解析消息数: %1 条\n"
                           u8"数据记录数: %2 条\n"
                           u8"耗时: %3")
        ...
        m_statusLabel->setText(tr(u8"解析失败: %1").arg(error));
        LOG_ERROR("MainWindow", QString::fromUtf8(u8"解析失败: %1").arg(error));
        
        QMessageBox::critical(this, tr(u8"解析错误"), 
            tr(u8"文件解析失败:\n%1\n\n"
               u8"请检查文件格式和解析器配置。").arg(error));
    }
}

void MainWindow::onDataLoaded(int count)
{
    m_countLabel->setText(tr(u8"数据: %1 条").arg(count));
}

void MainWindow::updateWindowTitle()
{
    QString title = tr(u8"1553B数据智能分析工具");
...
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"收到AI分析请求: %1").arg(query));
    
    QString modelId = m_aiQueryPanel->currentModelId();
    if (modelId.isEmpty()) {
        m_aiQueryPanel->appendResponse(tr(u8"错误: 未选择模型，请先在设置中配置模型。"));
    ...
    if (m_dataStore->totalCount() == 0) {
        m_aiQueryPanel->appendResponse(tr(u8"错误: 没有数据，请先导入1553B数据文件。"));
        m_aiQueryPanel->setLoading(false);
        return;
    }
    
    QString lowerQuery = query.toLower().trimmed();
    bool isSimpleQuery = lowerQuery == u8"所有" || lowerQuery == u8"全部" || 
                         lowerQuery == u8"清空" || lowerQuery == u8"重置" || 
                         lowerQuery == u8"取消筛选" || lowerQuery == u8"显示所有数据" ||
                         lowerQuery == u8"清除筛选";
...
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"收到AI聊天消息: %1").arg(message));
...
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"AI模式切换: %1").arg(mode == AIMode::Chat ? u8"聊天" : u8"分析"));
    m_statusLabel->setText(mode == AIMode::Chat ? tr(u8"AI聊天模式") : tr(u8"AI智能分析模式"));
}
...
void MainWindow::onAIQueryClear()
{
    ...
    m_dataStore->clearFilter();
    m_statusLabel->setText(tr(u8"筛选已清除"));
}
...
void MainWindow::onDataScopeChanged()
{
    ...
    QString scopeName;
    switch (scope) {
    case DataScope::AllData: scopeName = tr(u8"文件所有数据"); break;
    case DataScope::FilteredData: scopeName = tr(u8"当前筛选数据"); break;
    case DataScope::CurrentPage: scopeName = tr(u8"当前页数据"); break;
    }
    m_statusLabel->setText(tr(u8"数据范围: %1").arg(scopeName));
}
...
QString MainWindow::processSimpleQuery(const QString& query)
{
    ...
    if (lowerQuery.contains(u8"所有") || lowerQuery.contains(u8"全部") || lowerQuery.contains(u8"清空") || 
        lowerQuery.contains(u8"重置") || lowerQuery.contains(u8"取消筛选") || lowerQuery.contains(u8"清除筛选")) {
        m_dataStore->clearFilter();
        m_dataStore->clearTerminalFilter();
        m_dataStore->clearSubAddressFilter();
        m_dataStore->clearMessageTypeFilter();
        ...
        m_dataStore->clearTimeRange();
        m_dataModel->setDataStore(m_dataStore);
        return tr(u8"已清除所有筛选条件，显示全部 %1 条数据").arg(m_dataStore->totalCount());
    }
    
    return tr(u8"无法理解查询内容，请尝试更明确的表达，或切换到AI智能分析模式获取更智能的帮助。");
}
...
void MainWindow::executeAITools(const QJsonArray& toolCalls)
{
    ...
    if (hasFilterTool) {
        ...
        m_progressDialog->setStatusText(tr(u8"正在应用筛选条件..."));
        m_progressDialog->setProgress(0);
        m_progressDialog->startTimer();
        m_progressDialog->show();
        QCoreApplication::processEvents();
    ...
    for (const QJsonValue& call : toolCalls) {
        ...
        if (hasFilterTool && m_progressDialog) {
            m_progressDialog->setStatusText(tr(u8"正在执行: %1...").arg(toolName));
            m_progressDialog->setProgress(50);
            QCoreApplication::processEvents();
        }
        
        if (hadExistingFilter && (toolName == "query_data" || toolName == "apply_filters_batch")) {
            m_aiQueryPanel->appendResponse(tr(u8"↻ 已恢复原始数据，重新应用筛选条件..."));
        }
        ...
        if (result.success) {
            m_aiQueryPanel->appendResponse(tr(u8"✓ %1").arg(result.message));
        } else {
            m_aiQueryPanel->appendResponse(tr(u8"✗ 执行失败: %1").arg(result.message));
        }
    }
    ...
    if (hasFilterTool && m_progressDialog) {
        ...
        QString elapsedStr = elapsed < 1000 ? tr(u8"%1 毫秒").arg(elapsed) :
                             tr(u8"%1 秒").arg(QString::number(elapsed / 1000.0, 'f', 2));
        m_progressDialog->setStatusText(tr(u8"筛选完成！耗时 %1").arg(elapsedStr));
        m_progressDialog->stopTimer();
        m_progressDialog->setElapsedTime(elapsed);
        QCoreApplication::processEvents();
        QTimer::singleShot(800, this, [this]() {
        ...
        m_aiQueryPanel->appendResponse(tr(u8"⏱ 筛选耗时: %1").arg(elapsedStr));
    }
}

void MainWindow::onAIQueryDataRequested(const QJsonObject& filters)
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"执行数据查询"));
    m_dataModel->setDataStore(m_dataStore);
    m_centralTab->setCurrentIndex(0);
    m_statusLabel->setText(tr(u8"数据查询完成"));
}

void MainWindow::onAIGenerateChartRequested(const QString& chartType, const QString& subject, const QString& title)
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"生成图表: type=%1, subject=%2").arg(chartType, subject));
    
    /* 显示加载提示对话框 */
    if (!m_loadingDialog) {
        m_loadingDialog = new LoadingDialog(this);
    }
    m_loadingDialog->setText(tr(u8"正在加载统计图表..."));
    ...
    m_statusLabel->setText(tr(u8"图表生成完成"));
}

void MainWindow::onAIGenerateGanttRequested(const QJsonObject& filters)
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"生成甘特图"));
    
    /* 显示加载提示对话框 */
    if (!m_loadingDialog) {
        m_loadingDialog = new LoadingDialog(this);
    }
    m_loadingDialog->setText(tr(u8"正在加载甘特图..."));
    m_loadingDialog->show();
    QCoreApplication::processEvents();
    ...
    m_statusLabel->setText(tr(u8"甘特图生成完成"));
}

void MainWindow::onAIClearFilterRequested()
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"清除筛选条件"));
    m_dataStore->beginBatchFilterUpdate();
    m_dataStore->clearFilter();
    m_dataStore->clearTerminalFilter();
    m_dataStore->clearSubAddressFilter();
 ...
 */
void MainWindow::onAITimeIntervalAnalysisRequested(const TimeIntervalAnalysis& analysis)
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"时间间隔分析请求 - RT%1 SA%2, 记录数: %3")
    ...
    /* 设置图表数据源并绘制时间间隔折线图 */
    m_chartWidget->setDataStore(m_dataStore);
    m_chartWidget->drawTimeIntervalChart(analysis);

    /* 切换到图表选项卡 */
    m_centralTab->setCurrentIndex(2);
    m_statusLabel->setText(tr(u8"RT%1 子地址%2 时间间隔分析完成 - %3")
                           .arg(analysis.terminalAddress)
                           .arg(analysis.subAddress)
...
void MainWindow::showDataDetailDialog(int row)
{
    ...
    const PacketParser::SMbiMonPacketMsg& msg = m_dataStore->getMessage(record.msgIndex);
    const PacketParser::SMbiMonPacketHeader& hdr = msg.header;
    
    QDialog* dialog = new QDialog(this);
    dialog->setWindowTitle(tr(u8"数据详细 - 第%1条").arg(row + 1));
    ...
    auto createTable = [](QWidget* parent, int rows) {
        ...
        table->setHorizontalHeaderLabels(QStringList() << tr(u8"属性名称") << tr(u8"16进制") << tr(u8"10进制") << tr(u8"含义"));
    ...
    QLabel* sourceInfoLabel = new QLabel(tr(u8"完整数据源码 (%1 字节)").arg(fullSource.size()), sourceTab);
    ...
    tabWidget->addTab(sourceTab, tr(u8"源码"));
    ...
    QLabel* headerSourceLabel = new QLabel(tr(u8"包头源码 (%1 字节): %2").arg(headerSource.size()).arg(formatHexWithSpace(headerSource)), headerTab);
    ...
    tabWidget->addTab(headerTab, tr(u8"消息包头 (SMbiMonPacketHeader)"));
    
    setItem(headerTable, 0, "header1", QString("0x%1").arg(hdr.header1, 4, 16, QChar('0')).toUpper(), QString::number(hdr.header1), u8"固定值：A5 A5");
    setItem(headerTable, 1, "header2", QString("0x%1").arg(hdr.header2, 2, 16, QChar('0')).toUpper(), QString::number(hdr.header2), u8"固定值：A5");
    setItem(headerTable, 2, "mpuProduceId", QString("0x%1").arg(hdr.mpuProduceId, 2, 16, QChar('0')).toUpper(), QString::number(hdr.mpuProduceId), hdr.mpuProduceId == 1 ? "MPU1" : (hdr.mpuProduceId == 2 ? "MPU2" : u8"未知"));
    setItem(headerTable, 3, "packetLen", QString("0x%1").arg(hdr.packetLen, 4, 16, QChar('0')).toUpper(), QString::number(hdr.packetLen), u8"包总长度(字节)");
    setItem(headerTable, 4, "year", QString("0x%1").arg(hdr.year, 4, 16, QChar('0')).toUpper(), QString::number(hdr.year), QString::fromUtf8(u8"%1年").arg(hdr.year));
    setItem(headerTable, 5, "month", QString("0x%1").arg(hdr.month, 2, 16, QChar('0')).toUpper(), QString::number(hdr.month), QString::fromUtf8(u8"%1月").arg(hdr.month));
    setItem(headerTable, 6, "day", QString("0x%1").arg(hdr.day, 2, 16, QChar('0')).toUpper(), QString::number(hdr.day), QString::fromUtf8(u8"%1日").arg(hdr.day));
    ...
    setItem(headerTable, 7, "timestamp", QString("0x%1").arg(ts40us, 8, 16, QChar('0')).toUpper(), QString::number(ts40us), QString::fromUtf8(u8"时间戳(40us) = %1").arg(timeStr));
    ...
    QLabel* dataCountLabel = new QLabel(tr(u8"本消息包含 %1 个数据包").arg(msg.packetDatas.size()), dataTab);
    ...
    for (int dataIdx = 0; dataIdx < msg.packetDatas.size(); ++dataIdx) {
        ...
        QGroupBox* dataGroup = new QGroupBox(tr(u8"数据包 #%1 (索引: %2)").arg(dataIdx + 1).arg(dataIdx), scrollContent);
        ...
        if (isCurrentData) {
            ...
            dataGroup->setTitle(tr(u8"数据包 #%1 (索引: %2) - 当前选中").arg(dataIdx + 1).arg(dataIdx));
        }
        
        QTableWidget* dataTable = createTable(dataGroup, 8);
        groupLayout->addWidget(dataTable);
        
        setItem(dataTable, 0, "header", QString("0x%1").arg(pkt.header, 4, 16, QChar('0')).toUpper(), QString::number(pkt.header), u8"固定值：AA BB");
        setItem(dataTable, 1, "cmd1", QString("0x%1").arg(pkt.cmd1, 4, 16, QChar('0')).toUpper(), QString::number(pkt.cmd1), u8"命令字1");
        setItem(dataTable, 2, "cmd2", QString("0x%1").arg(pkt.cmd2, 4, 16, QChar('0')).toUpper(), QString::number(pkt.cmd2), u8"命令字2");
        setItem(dataTable, 3, "states1", QString("0x%1").arg(pkt.states1, 4, 16, QChar('0')).toUpper(), QString::number(pkt.states1), u8"状态字1");
        setItem(dataTable, 4, "states2", QString("0x%1").arg(pkt.states2, 4, 16, QChar('0')).toUpper(), QString::number(pkt.states2), u8"状态字2");
        setItem(dataTable, 5, "chstt", QString("0x%1").arg(pkt.chstt, 4, 16, QChar('0')).toUpper(), QString::number(pkt.chstt), pkt.chstt ? u8"成功" : u8"失败");
        ...
        setItem(dataTable, 6, "timestamp", QString("0x%1").arg(dataTs40us, 8, 16, QChar('0')).toUpper(), QString::number(dataTs40us), QString::fromUtf8(u8"时间戳(40us) = %1").arg(dataTimeStr));
        ...
        QLabel* pktSourceLabel = new QLabel(tr(u8"源码 (%1 字节): %2").arg(pktSource.size()).arg(formatHexWithSpace(pktSource)), dataGroup);
    ...
    tabWidget->addTab(dataTab, tr(u8"数据包 (SMbiMonPacketData)"));
    ...
    QGroupBox* cmd1Group = new QGroupBox(tr(u8"CMD1 命令字1 (0x%1)").arg(currentPkt.cmd1, 4, 16, QChar('0')).toUpper(), cmdTab);
    ...
    setItem(cmd1Table, 0, "zhongduandizhi", QString("0x%1").arg(cmd1.zhongduandizhi, 2, 16, QChar('0')).toUpper(), QString::number(cmd1.zhongduandizhi), QString::fromUtf8(u8"终端地址 (bit15-11) %1").arg(cmd1.zhongduandizhi == 31 ? u8"= 广播" : u8""));
    setItem(cmd1Table, 1, "T_R", QString("0x%1").arg(cmd1.T_R, 1, 16, QChar('0')).toUpper(), QString::number(cmd1.T_R), cmd1.T_R ? u8"RT→BC (bit10=1)" : u8"BC→RT (bit10=0)");
    setItem(cmd1Table, 2, "zidizhi", QString("0x%1").arg(cmd1.zidizhi, 2, 16, QChar('0')).toUpper(), QString::number(cmd1.zidizhi), QString::fromUtf8(u8"子地址 (bit9-5) %1").arg(cmd1.zidizhi == 0 || cmd1.zidizhi == 31 ? u8"= datas长度2字节" : ""));
    setItem(cmd1Table, 3, "sjzjs_fsdm", QString("0x%1").arg(cmd1.sjzjs_fsdm, 2, 16, QChar('0')).toUpper(), QString::number(cmd1.sjzjs_fsdm), QString::fromUtf8(u8"数据计数/发送码 (bit4-0) %1").arg(cmd1.sjzjs_fsdm == 0 ? u8"= datas长度64字节" : QString::fromUtf8(u8"= datas长度%1字节").arg(cmd1.sjzjs_fsdm * 2)));
    cmd1Table->resizeColumnsToContents();
    
    QGroupBox* cmd2Group = new QGroupBox(tr(u8"CMD2 命令字2 (0x%1)").arg(currentPkt.cmd2, 4, 16, QChar('0')).toUpper(), cmdTab);
    ...
    setItem(cmd2Table, 0, "zhongduandizhi", QString("0x%1").arg(cmd2.zhongduandizhi, 2, 16, QChar('0')).toUpper(), QString::number(cmd2.zhongduandizhi), QString::fromUtf8(u8"终端地址 (bit15-11) %1").arg(cmd2.zhongduandizhi == 31 ? u8"= 广播" : ""));
    setItem(cmd2Table, 1, "T_R", QString("0x%1").arg(cmd2.T_R, 1, 16, QChar('0')).toUpper(), QString::number(cmd2.T_R), cmd2.T_R ? u8"RT→BC (bit10=1)" : u8"BC→RT (bit10=0)");
    setItem(cmd2Table, 2, "zidizhi", QString("0x%1").arg(cmd2.zidizhi, 2, 16, QChar('0')).toUpper(), QString::number(cmd2.zidizhi), QString::fromUtf8(u8"子地址 (bit9-5) %1").arg(cmd2.zidizhi == 0 || cmd2.zidizhi == 31 ? u8"= datas长度2字节" : ""));
    setItem(cmd2Table, 3, "sjzjs_fsdm", QString("0x%1").arg(cmd2.sjzjs_fsdm, 2, 16, QChar('0')).toUpper(), QString::number(cmd2.sjzjs_fsdm), QString::fromUtf8(u8"数据计数/发送码 (bit4-0) %1").arg(cmd2.sjzjs_fsdm == 0 ? u8"= datas长度64字节" : QString::fromUtf8(u8"= datas长度%1字节").arg(cmd2.sjzjs_fsdm * 2)));
    cmd2Table->resizeColumnsToContents();
    
    tabWidget->addTab(cmdTab, tr(u8"命令字 (CMD)"));
    ...
    tabWidget->addTab(metaTab, tr(u8"元信息"));
    
    setItem(metaTable, 0, "msgIndex", "-", QString::number(record.msgIndex), u8"消息索引");
    setItem(metaTable, 1, "dataIndex", "-", QString::number(record.dataIndex), u8"数据索引");
    setItem(metaTable, 2, "rowIndex", "-", QString::number(record.rowIndex), u8"行索引");
    setItem(metaTable, 3, "messageType", "-", "-", messageTypeToString(record.messageType));
    setItem(metaTable, 4, "timestampMs", "-", QString::number(record.timestampMs, 'f', 3), u8"时间戳(毫秒)");
...
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"AI查询开始"));
    m_elapsedTimer.start();
    m_aiQueryPanel->setLoading(true);
    m_statusLabel->setText(tr(u8"正在等待AI响应..."));
}

void MainWindow::onModelQueryFinished(const ModelResponse& response)
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"AI查询完成, 成功=%1").arg(response.success));
    m_aiQueryPanel->setLoading(false);
    
    qint64 aiElapsed = m_elapsedTimer.elapsed();
    QString elapsedStr = aiElapsed < 1000 ? tr(u8"%1 毫秒").arg(aiElapsed) :
                         tr(u8"%1 秒").arg(QString::number(aiElapsed / 1000.0, 'f', 2));
    ...
    if (response.success) {
        ...
        if (!toolCalls.isEmpty()) {
            m_aiQueryPanel->appendResponse(tr(u8"⏱ AI响应耗时: %1").arg(elapsedStr));
            executeAITools(toolCalls);
        } else {
            m_aiQueryPanel->appendResponse(content);
            m_aiQueryPanel->appendResponse(tr(u8"\n⏱ 响应耗时: %1").arg(elapsedStr));
        }
        
        m_statusLabel->setText(tr(u8"AI响应完成 (耗时 %1)").arg(elapsedStr));
    } else {
        m_aiQueryPanel->appendResponse(tr(u8"错误: %1").arg(response.error));
        m_statusLabel->setText(tr(u8"AI响应失败"));
    }
}

void MainWindow::onModelQueryError(const QString& error)
{
    LOG_ERROR("MainWindow", QString::fromUtf8(u8"AI查询错误: %1").arg(error));
    m_aiQueryPanel->setLoading(false);
    m_aiQueryPanel->appendResponse(tr(u8"错误: %1").arg(error));
    m_statusLabel->setText(tr(u8"查询失败"));
    
    QMessageBox::warning(this, tr(u8"AI连接失败"), 
        QString(tr(u8"无法连接到AI服务：\n%1\n\n请检查：\n1. 网络连接是否正常\n2. API密钥是否正确配置\n3. API服务是否可用")).arg(error));
}
            ...
            if (braceCount == 0 && startIndex >= 0) {
                ...
                if (error.error == QJsonParseError::NoError && doc.isObject()) {
                    ...
                    if (obj.contains("name")) {
                        ...
                        LOG_INFO("MainWindow", QString::fromUtf8(u8"提取到工具调用: %1, 参数: %2")
...
{
    if (m_dataStore->totalCount() == 0) {
        QMessageBox::warning(this, tr(u8"无法生成报告"), tr(u8"没有数据可分析，请先导入数据文件。"));
        return;
    }
    ...
    if (defaultFormat == "pdf") {
        filterStr = tr(u8"PDF报告 (*.pdf);;HTML报告 (*.html);;DOCX报告 (*.docx)");
        defaultFilterIndex = 0;
    } else if (defaultFormat == "docx") {
        filterStr = tr(u8"DOCX报告 (*.docx);;HTML报告 (*.html);;PDF报告 (*.pdf)");
        defaultFilterIndex = 0;
    } else {
        filterStr = tr(u8"HTML报告 (*.html);;PDF报告 (*.pdf);;DOCX报告 (*.docx)");
        defaultFilterIndex = 0;
    }
    
    QString defaultPath = QDir(QDir::homePath()).filePath(u8"1553B总线数据分析报告");
    ...
    if (!provider) {
        QMessageBox::warning(this, tr(u8"无法生成报告"), 
            tr(u8"未配置AI模型，无法进行深度分析。\n请先在设置中配置AI模型。"));
        return;
    }
    ...
    m_reportProgressDialog = new QProgressDialog(tr(u8"正在生成智能分析报告..."), tr(u8"取消"), 0, 100, this);
    m_reportProgressDialog->setWindowTitle(tr(u8"报告生成中"));
    ...
    m_reportProgressDialog->setLabelText(tr(u8"初始化..."));
    
    connect(m_reportProgressDialog, &QProgressDialog::canceled, this, [this]() {
        m_statusLabel->setText(tr(u8"报告生成已取消"));
        LOG_INFO("MainWindow", QString::fromUtf8(u8"用户取消了报告生成"));
        if (m_reportGenerator) {
            m_reportGenerator->cancelReport();
        }
    });
    
    m_statusLabel->setText(tr(u8"正在生成智能分析报告..."));
    LOG_INFO("MainWindow", QString::fromUtf8(u8"开始生成智能分析报告: %1").arg(filePath));
...
void MainWindow::onReportProgress(int percent, const QString& stage, double elapsedSeconds)
{
    if (m_reportProgressDialog) {
        ...
        QString timeStr;
        if (elapsedSeconds > 0) {
            timeStr = QString::fromUtf8(u8" (%1秒)").arg(elapsedSeconds, 0, 'f', 1);
        }
        
        m_reportProgressDialog->setLabelText(tr("%1%2").arg(stage).arg(timeStr));
    }
    
    m_statusLabel->setText(tr(u8"报告生成: %1 (%2%)").arg(stage).arg(percent));
}
...
void MainWindow::onReportFinished(bool success, const QString& filePath)
{
    ...
    if (success) {
        m_statusLabel->setText(tr(u8"报告生成成功"));
        LOG_INFO("MainWindow", QString::fromUtf8(u8"报告生成成功: %1").arg(filePath));
        
        QMessageBox msgBox(this);
        msgBox.setWindowTitle(tr(u8"报告生成成功"));
        msgBox.setText(tr(u8"智能分析报告已生成完成！"));
        msgBox.setInformativeText(tr(u8"文件路径: %1").arg(filePath));
        msgBox.setStandardButtons(QMessageBox::Open | QMessageBox::Ok);
        msgBox.setDefaultButton(QMessageBox::Open);
        ...
        m_statusLabel->setText(tr(u8"报告生成失败"));
        QString error = m_reportGenerator->lastError();
        LOG_ERROR("MainWindow", QString::fromUtf8(u8"报告生成失败: %1").arg(error));
        QMessageBox::warning(this, tr(u8"报告生成失败"), tr(u8"无法生成报告: %1").arg(error));
    }
}
...
{
    // 尝试1: 相对路径（开发环境）
    QFile styleFile("src/ui/styles/dark_theme.qss");
    if (styleFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QTextStream stream(&styleFile);
        QString style = stream.readAll();
        qApp->setStyleSheet(style);
        styleFile.close();
        LOG_INFO("MainWindow", QString::fromUtf8(u8"深色主题样式加载成功（相对路径）"));
        return;
    }
    
    // 尝试2: 绝对路径（从应用程序目录向上两级）
    QDir appDir = QDir(QCoreApplication::applicationDirPath());
    if (appDir.cdUp() && appDir.cdUp()) {
        QString absolutePath = appDir.absoluteFilePath("src/ui/styles/dark_theme.qss");
        QFile absoluteStyleFile(absolutePath);
        if (absoluteStyleFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
            QTextStream stream(&absoluteStyleFile);
            QString style = stream.readAll();
            qApp->setStyleSheet(style);
            absoluteStyleFile.close();
            LOG_INFO("MainWindow", QString::fromUtf8(u8"深色主题样式加载成功（绝对路径）: %1").arg(absolutePath));
            return;
        }
    }
    ...
    QString currentPath = currentDir.absoluteFilePath("src/ui/styles/dark_theme.qss");
    QFile currentStyleFile(currentPath);
    if (currentStyleFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QTextStream stream(&currentStyleFile);
        QString style = stream.readAll();
        qApp->setStyleSheet(style);
        currentStyleFile.close();
        LOG_INFO("MainWindow", QString::fromUtf8(u8"深色主题样式加载成功（当前目录）: %1").arg(currentPath));
        return;
    }
    
    // 尝试4: 直接在bin目录中查找
    QString binPath = QCoreApplication::applicationDirPath() + "/dark_theme.qss";
    QFile binStyleFile(binPath);
    if (binStyleFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QTextStream stream(&binStyleFile);
        QString style = stream.readAll();
        qApp->setStyleSheet(style);
        binStyleFile.close();
        LOG_INFO("MainWindow", QString::fromUtf8(u8"深色主题样式加载成功（bin目录）: %1").arg(binPath));
        return;
    }
    
    // 所有尝试都失败
    LOG_ERROR("MainWindow", QString::fromUtf8(u8"无法加载深色主题样式文件"));
}

void MainWindow::onGenerateTestData()
{
    QString dir = QFileDialog::getExistingDirectory(this, tr(u8"选择测试数据保存目录"),
                                                     QDir::currentPath(),
    ...
    QDialog* dialog = new QDialog(this);
    dialog->setWindowTitle(tr(u8"生成测试数据"));
    dialog->setMinimumWidth(500);
    
    QVBoxLayout* layout = new QVBoxLayout(dialog);
    
    QGroupBox* byteOrderGroup = new QGroupBox(tr(u8"字节序设置"));
    QHBoxLayout* byteOrderLayout = new QHBoxLayout(byteOrderGroup);
    QRadioButton* littleEndianRadio = new QRadioButton(tr(u8"小端序"));
    QRadioButton* bigEndianRadio = new QRadioButton(tr(u8"大端序"));
    ...
    endDateLayout->addWidget(new QLabel(tr(u8"结束日期:")));
    ...
    endTimeLayout->addWidget(new QLabel(tr(u8"结束时间:")));
    ...
    QLabel* label = new QLabel(tr(u8"选择要生成的测试数据规模："));
    layout->addWidget(label);
    
    QCheckBox* check10k = new QCheckBox(tr(u8"1万条数据"));
    QCheckBox* check50k = new QCheckBox(tr(u8"5万条数据"));
    QCheckBox* check200k = new QCheckBox(tr(u8"20万条数据"));
    QCheckBox* check500k = new QCheckBox(tr(u8"50万条数据"));
    QCheckBox* check1m = new QCheckBox(tr(u8"100万条数据"));
    ...
    if (dialog->exec() == QDialog::Accepted) {
        ...
        if (counts.isEmpty()) {
            QMessageBox::warning(this, tr(u8"提示"), tr(u8"请至少选择一个数据规模"));
            return;
        }
        ...
        QProgressDialog* progress = new QProgressDialog(tr(u8"正在生成测试数据..."), tr(u8"取消"), 0, counts.size(), this);
        progress->setWindowModality(Qt::WindowModal);
        progress->setMinimumDuration(0);
        progress->setValue(0);
        ...
        for (int i = 0; i < counts.size(); i++) {
            ...
            progress->setLabelText(tr(u8"正在生成 %1 万条数据...").arg(counts[i] / 10000));
            progress->setValue(i);
            QCoreApplication::processEvents();
        ...
        if (successCount == counts.size()) {
            QMessageBox::information(this, tr(u8"完成"), tr(u8"成功生成 %1 个测试数据文件").arg(successCount));
        } else {
            QMessageBox::warning(this, tr(u8"部分完成"), tr(u8"成功生成 %1/%2 个测试数据文件").arg(successCount).arg(counts.size()));
        }
    }
...
{
    QString filePath = QFileDialog::getOpenFileName(this, tr(u8"选择要查看的文件"),
                                                     QDir::currentPath(),
                                                     tr(u8"二进制文件 (*.bin);;所有文件 (*.*)"));
    if (filePath.isEmpty()) {
        return;
    }
...
void MainWindow::onPageChanged(int currentPage, int totalPages, int filteredCount)
{
    ...
    // 更新状态栏
    m_countLabel->setText(tr(u8"数据: %1 条").arg(filteredCount));
...
void MainWindow::onPageSizeChanged(int size)
{
    ...
    LOG_INFO("MainWindow", QString::fromUtf8(u8"每页条数已更改为: %1").arg(size));
}
...
void MainWindow::onProgressDialogCanceled()
{
    ...
    if (m_progressDialog) {
        m_progressDialog->stopTimer();
        m_progressDialog->setStatusText(tr(u8"操作已取消"));
        m_progressDialog->hide();
        m_progressDialog->reset();
    }
 ...
 * 当甘特图更新完成时关闭加载提示对话框
 */
void MainWindow::onGanttUpdateFinished()
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"甘特图更新完成"));
 ...
 * 当图表更新完成时关闭加载提示对话框
 */
void MainWindow::onChartUpdateFinished()
{
    LOG_INFO("MainWindow", QString::fromUtf8(u8"图表更新完成"));
