...
void ProgressDialog::setupUI()
{
    ...
    // 状态标签
    m_statusLabel = new QLabel(tr(u8"正在初始化..."));
    ...
    // 耗时标签
    m_timeLabel = new QLabel(tr(u8"已用时间: 00:00:00"));
    ...
    m_cancelButton = new QPushButton(tr(u8"取消"));
...
void ProgressDialog::setElapsedTimeFormatted(qint64 milliseconds)
{
    if (m_timeLabel) {
        ...
        QString displayText = tr(u8"已用时间: %1").arg(timeStr);
...
void ProgressDialog::onCancelClicked()
{
    ...
    m_cancelButton->setText(tr(u8"取消中..."));
    m_statusLabel->setText(tr(u8"正在取消操作..."));
