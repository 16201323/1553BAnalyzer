...
BinaryParser::BinaryParser(QObject *parent)
    ...
    , m_canceled(false)
{
    LOG_DEBUG("BinaryParser", u8"BinaryParser实例创建");
}
...
{
    LOG_DEBUG("BinaryParser", u8"BinaryParser实例销毁");
}
...
void BinaryParser::setByteOrder(bool littleEndian)
{
    ...
    LOG_DEBUG("BinaryParser", QString::fromUtf8(u8"字节序设置为: %1").arg(littleEndian ? u8"小端" : u8"大端"));
}
...
{
    m_header1 = value;
    LOG_DEBUG("BinaryParser", QString::fromUtf8(u8"包头1设置为: 0x%1").arg(value, 4, 16, QChar('0')));
}
...
void BinaryParser::setHeader2(quint16 value)
{
    ...
    LOG_DEBUG("BinaryParser", QString::fromUtf8(u8"包头2设置为: 0x%1").arg(m_header2, 2, 16, QChar('0')));
}
...
{
    m_dataHeader = value;
    LOG_DEBUG("BinaryParser", QString::fromUtf8(u8"数据头设置为: 0x%1").arg(value, 4, 16, QChar('0')));
}
...
void BinaryParser::setMaxErrorTolerance(int count)
{
    ...
    LOG_DEBUG("BinaryParser", QString::fromUtf8(u8"最大错误容差设置为: %1").arg(count));
}

void BinaryParser::cancel()
{
    m_canceled.store(true);
    LOG_INFO("BinaryParser", QString::fromUtf8(u8"解析取消标志已设置"));
}

bool BinaryParser::isCanceled() const
{
...
{
    LOG_INFO("BinaryParser", QString::fromUtf8(u8"开始解析文件: %1").arg(filePath));
    ...
    if (!fileInfo.exists()) {
        m_lastError = tr(u8"文件不存在: %1").arg(filePath);
    ...
    LOG_DEBUG("BinaryParser", QString::fromUtf8(u8"文件大小: %1 字节").arg(file.size()));
    ...
    while (!stream.atEnd()) {
        if (m_canceled.load()) {
            m_lastError = tr(u8"解析已取消，已解析: %1 条消息").arg(parsedCount);
            LOG_INFO("BinaryParser", m_lastError);
            break;
        }
        ...
        PacketParser::SMbiMonPacketMsg msg;
        
        if (!parseHeader(stream, msg.header)) {
            errorCount++;
            
            if (errorCount > m_maxErrorTolerance) {
                m_lastError = tr(u8"解析错误超过容差限制，已解析: %1 条消息").arg(parsedCount);
        ...
        int dataLen = msg.header.packetLen - sizeof(PacketParser::SMbiMonPacketHeader);
        if (dataLen <= 0) {
            LOG_WARNING("BinaryParser", QString::fromUtf8(u8"无效的数据长度: %1, packetLen: %2")
                        .arg(dataLen)
        ...
        int estimatedCount = dataLen / PacketParser::SMbiMonPacketDataFixedPartSize;
        if (estimatedCount == 0) {
            estimatedCount = 1;
        }
        
        // LOG_DEBUG("BinaryParser", QString::fromUtf8(u8"解析消息 #%1, 数据长度: %2, 预估数据条数: %3")
        //           .arg(parsedCount + 1)
        //           .arg(dataLen)
        //           .arg(estimatedCount));
        ...
        while (bytesRead < dataLen) {
            PacketParser::SMbiMonPacketData pktData;
            int pktSize = 0;
            if (parsePacketData(stream, pktData, dataLen - bytesRead, pktSize)) {
                ...
                LOG_WARNING("BinaryParser", QString::fromUtf8(u8"数据包解析失败，已读取 %1 字节，跳过剩余 %2 字节")
    ...
    LOG_INFO("BinaryParser", QString::fromUtf8(u8"文件解析完成，共解析 %1 条消息，%2 条数据记录")
              .arg(parsedCount)
              .arg(totalDataCount));
...
bool BinaryParser::parsePacketData(QDataStream& stream, SMbiMonPacketData& data, int remainingLen, int& bytesRead)
{
    ...
    if (remainingLen - bytesRead < datasLen) {
        LOG_WARNING("BinaryParser", QString::fromUtf8(u8"剩余数据不足: 需要datas %1字节, 剩余%2字节")
                    .arg(datasLen)
    ...
    if (readResult != datasLen) {
        LOG_WARNING("BinaryParser", QString::fromUtf8(u8"数据读取不完整: 期望%1字节, 实际读取%2字节")
                    .arg(datasLen)
                    .arg(readResult));
        return false;
    }
 ...
 * @return 解析后的消息列表
 */
QVector<PacketParser::SMbiMonPacketMsg> BinaryParser::getParsedData() const
{
    return m_parsedData;
}
