                                              ...
                                              quint32 seed)
{
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        LOG_ERROR("TestDataGenerator", QString::fromUtf8(u8"无法打开文件: %1").arg(filename));
        return false;
    }

    QDataStream out(&file);
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"生成测试文件的大小端:") + byteOrder);
    out.setByteOrder(byteOrder == "big" ? QDataStream::BigEndian : QDataStream::LittleEndian);

    QT5COMPAT_SET_RANDOM_SEED(seed);
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"随机数种子: %1").arg(seed == 0 ? u8"自动（当前时间）" : QString::number(seed)));
    ...
    for (int i = 0; i < recordCount; i++) {
        ...
        int packetsInThisMsg = QT5COMPAT_RANDOM_INT(1, 6);
        ...
        for (int p = 0; p < packetsInThisMsg; p++) {
            ...
            case 0:
                msgTypeCount[0]++;
                terminalAddr1 = QT5COMPAT_RANDOM_INT(0, 31);
                subAddr1 = QT5COMPAT_RANDOM_INT(1, 31);
                isSend1 = false;
                dataCount1 = QT5COMPAT_RANDOM_INT(1, 32);
            ...
            case 2:
                msgTypeCount[2]++;
                terminalAddr1 = QT5COMPAT_RANDOM_INT(0, 31);
                terminalAddr2 = QT5COMPAT_RANDOM_INT(0, 31);
                while (terminalAddr2 == terminalAddr1) {
                    terminalAddr2 = QT5COMPAT_RANDOM_INT(0, 31);
                }
                subAddr1 = QT5COMPAT_RANDOM_INT(1, 31);
                subAddr2 = QT5COMPAT_RANDOM_INT(1, 31);
                dataCount1 = QT5COMPAT_RANDOM_INT(1, 32);
            ...
            case 4:
                ...
                terminalAddr1 = QT5COMPAT_RANDOM_INT(0, 31);
                subAddr1 = 0;
                isSend1 = QT5COMPAT_RANDOM_INT(0, 2) == 1;
                dataCount1 = QT5COMPAT_RANDOM_INT(0, 32);
                cmd1 = generateCmd1(terminalAddr1, subAddr1, isSend1, dataCount1);
                cmd2 = 0;
                break;
            }
            ...
            int statusType = QT5COMPAT_RANDOM_INT(0, 100);
            if (statusType < 70) {
                statusTypeCount[0]++;
            ...
            for (int j = 0; j < datasLen; j++) {
                datas[j] = static_cast<char>(QT5COMPAT_RANDOM_INT(0, 256));
            }
    ...
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"生成测试数据文件: %1").arg(filename));
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"  字节序: %1").arg(byteOrder == "big" ? u8"大端" : u8"小端"));
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"  总消息数: %1").arg(recordCount));
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"  总数据包: %1").arg(totalPacketCount));
    LOG_INFO("TestDataGenerator", QString(u8"  BC->RT: %1").arg(msgTypeCount[0]));
    LOG_INFO("TestDataGenerator", QString(u8"  RT->BC: %1").arg(msgTypeCount[1]));
    LOG_INFO("TestDataGenerator", QString(u8"  RT->RT: %1").arg(msgTypeCount[2]));
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"  广播: %1").arg(msgTypeCount[3]));
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"  模式命令: %1").arg(msgTypeCount[4]));
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"  状态:成功: %1").arg(statusTypeCount[0]));
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"  状态:失败: %1").arg(statusTypeCount[1]));
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"  状态:错误标志: %1").arg(statusTypeCount[2]));
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"  状态:部分失败: %1").arg(statusTypeCount[3]));
    LOG_INFO("TestDataGenerator", QString::fromUtf8(u8"  文件大小: %1 MB").arg(fileSize / 1024.0 / 1024.0, 0, 'f', 2));
