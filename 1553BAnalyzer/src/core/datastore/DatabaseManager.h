...
typedef QHash<MessageType, qint64> MessageTypeDistribution;
...
public:
     ...
     */
    MessageTypeDistribution queryMessageTypeDistribution(int fileId);
...
private:
    ...
    struct ImportContext {
        ...
        int startIdx;                           // 起始索引
        int endIdx;                             // 结束索引
        QVector<PacketParser::SMbiMonPacketMsg> data;         // 数据
        int fileId;                             // 文件ID
     ...
     */
    QVector<DbDataRecord> convertToRecords(const PacketParser::SMbiMonPacketMsg& msg, int fileId, int msgIndex, int& globalIndex);
...
Q_DECLARE_METATYPE(MessageTypeDistribution)
