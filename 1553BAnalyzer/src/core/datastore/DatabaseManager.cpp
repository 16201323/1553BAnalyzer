...
// 批量插入大小
constexpr int BATCH_INSERT_SIZE = 30;  // 限制批次大小，避免SQLite参数数量限制 (默认999)
...
{
    if (m_initialized) {
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 已初始化"));
        return true;
    }
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 数据库路径: %1").arg(m_dbPath));
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 应用目录: %1").arg(QCoreApplication::applicationDirPath()));
    
    bool dbExists = QFile::exists(m_dbPath);
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 数据库文件是否存在: %1").arg(dbExists));
    ...
    if (!m_writeDb.open()) {
        logError(tr(u8"无法打开数据库: %1").arg(m_writeDb.lastError().text()));
        return false;
    }
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 写连接已打开"));
    ...
    for (int i = 0; i < m_readConnectionCount; ++i) {
        ...
        if (!readDb.open()) {
            logError(tr(u8"无法打开读数据库连接: %1").arg(readDb.lastError().text()));
            continue;
        }
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 读连接数: %1").arg(readCreated));
    
    if (!dbExists) {
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 创建新数据库表..."));
        if (!createTables()) {
            logError(tr(u8"创建数据库表失败"));
            return false;
        }
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 数据表已创建"));
        
        if (!createColumnarTables()) {
            logError(tr(u8"创建列式存储表失败"));
            return false;
        }
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 列式存储表已创建"));
        
        if (!createIndexes()) {
            logError(tr(u8"创建索引失败"));
            return false;
        }
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 索引已创建"));
        
        if (!createViews()) {
            logError(tr(u8"创建视图失败"));
            return false;
        }
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 视图已创建"));
        
        if (!initDatabaseVersion()) {
            logError(tr(u8"初始化数据库版本失败"));
            return false;
        }
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 版本已初始化"));
    } else {
        int currentVersion = getDatabaseVersion();
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 现有数据库版本: %1").arg(currentVersion));
        if (currentVersion < DATABASE_VERSION) {
            if (!upgradeDatabase(DATABASE_VERSION)) {
                logError(tr(u8"升级数据库失败"));
                return false;
            }
        }
    }
    ...
    
    {
        ...
        for (const QString& tempFile : tempFiles) {
            ...
            if (QFile::remove(fullPath)) {
                LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 清理残留临时文件: %1").arg(tempFile));
            }
        }
    }
    
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"数据库初始化成功"));
    return true;
}
...
bool DatabaseManager::upgradeDatabase(int targetVersion)
{
    ...
    // 开始事务
    if (!db.transaction()) {
        logError(tr(u8"开始事务失败: %1").arg(db.lastError().text()));
        return false;
    }
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 升级数据库 - 当前版本: %1, 目标版本: %2").arg(currentVersion).arg(targetVersion));
    
    // 版本1到版本2：添加msg_index列
    if (currentVersion < 2) {
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 执行升级脚本: 版本1 -> 版本2 (添加msg_index列)"));
        ...
        if (!columnExists) {
            if (!query.exec("ALTER TABLE packets ADD COLUMN msg_index INTEGER NOT NULL DEFAULT 0")) {
                LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 添加msg_index列失败: %1").arg(query.lastError().text()));
                db.rollback();
                return false;
            }
            LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] msg_index列添加成功"));
        } else {
            LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] msg_index列已存在，跳过添加"));
        }
    }
    
    if (currentVersion < 3) {
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 执行升级脚本: 版本2 -> 版本3 (添加packetHeader字段)"));
        ...
        for (const QString& sql : columnsToAdd) {
            ...
            if (!colExists) {
                if (!query.exec(sql)) {
                    LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 添加%1列失败: %2").arg(colName).arg(query.lastError().text()));
                    db.rollback();
                    return false;
                }
                LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] %1列添加成功").arg(colName));
            }
        }
    }
    
    if (currentVersion < 4) {
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 执行升级脚本: 版本3 -> 版本4 (添加header_timestamp列)"));
        ...
        if (!colExists) {
            if (!query.exec("ALTER TABLE packets ADD COLUMN header_timestamp INTEGER DEFAULT 0")) {
                LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 添加header_timestamp列失败: %1").arg(query.lastError().text()));
                db.rollback();
                return false;
            }
            
            query.exec("UPDATE packets SET header_timestamp = packet_timestamp WHERE header_timestamp = 0");
            LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] header_timestamp列添加成功"));
        } else {
            LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] header_timestamp列已存在，跳过添加"));
        }
    }
    ...
    if (!query.exec()) {
        db.rollback();
        logError(tr(u8"更新版本号失败: %1").arg(query.lastError().text()));
        return false;
    }
    ...
    if (!db.commit()) {
        db.rollback();
        logError(tr(u8"提交事务失败: %1").arg(db.lastError().text()));
        return false;
    }
    
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 数据库升级完成，版本: %1").arg(targetVersion));
    return true;
}
...
bool DatabaseManager::initDatabaseVersion()
{
    ...
    if (!query.exec()) {
        logError(tr(u8"初始化数据库版本失败: %1").arg(query.lastError().text()));
        return false;
    }
...
void DatabaseManager::cancelImport()
{
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 导入取消已请求"));
}

bool DatabaseManager::isImportCanceled() const
{
    return m_importCanceled.load();
}
...
{
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 开始并行导入, 数据量: %1, 文件路径: %2, 已初始化: %3")
             .arg(data.size()).arg(filePath).arg(m_initialized));
    
    m_importCanceled.store(false);
    
    if (!m_initialized) {
        logError(tr(u8"数据库未初始化"));
        return -1;
    }
    ...
    if (existingFileId > 0) {
        ...
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 文件已存在, 文件ID: %1, 消息数: %2, 数据包数: %3")
                 .arg(existingFileId).arg(messageCount).arg(packetCount));
        
        if (messageCount == data.size() && packetCount > 0) {
            LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 数据完整, 返回现有文件ID: %1").arg(existingFileId));
            return existingFileId;
        }
        
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 数据不完整(预期消息: %1, 实际消息: %2), 删除旧数据重新导入")
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 文件信息 - 名称: %1, 大小: %2, 哈希: %3")
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 事务已开始"));
    ...
    if (!query.exec()) {
        db.rollback();
        logError(tr(u8"插入文件记录失败: %1").arg(query.lastError().text()));
        return -1;
    }
    
    int fileId = query.lastInsertId().toInt();
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 文件记录已插入, 文件ID: %1").arg(fileId));
    
    {
        ...
        if (!updateQuery.exec()) {
            LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 更新is_current失败: %1").arg(updateQuery.lastError().text()));
        }
    }
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 导入参数 - 线程数: %1, 消息总数: %2, 分块大小: %3")
             .arg(threadCount).arg(totalMessages).arg(chunkSize));
    
    if (progressCallback) {
        progressCallback(0, totalMessages, tr(u8"准备并行导入..."));
    }
    ...
    for (int i = 0; i < threadCount; ++i) {
        ...
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 上下文 %1 - 起始索引: %2, 结束索引: %3, 数据量: %4, 临时库: %5")
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 所有导入线程已完成"));
    ...
    if (m_importCanceled.load()) {
        ...
        db.rollback();
        m_importCanceled.store(false);
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 导入已取消"));
        return -1;
    }
    
    if (progressCallback) {
        // 并行导入已完成，进度达到80%，开始合并阶段
        progressCallback(totalMessages * 8 / 10, totalMessages, tr(u8"合并数据..."));
    }
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 事务已提交，开始合并临时数据库..."));
    
    // 合并临时数据库到主数据库
    mergeTempDatabases(tempDbs, fileId, progressCallback, totalMessages);
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并完成"));
    ...
    for (const QString& tempDb : tempDbs) {
        if (QFile::exists(tempDb)) {
            ...
            if (!removed) {
                LOG_WARNING("DatabaseManager", QString::fromUtf8(u8"[数据库] 无法删除临时数据库文件: %1").arg(tempDb));
            }
        }
    }
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并后 - 消息数: %1, 数据包数: %2, 耗时: %3 毫秒")
    ...
    updateQuery.addBindValue(fileId);
    if (!updateQuery.exec()) {
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 更新文件统计失败: %1").arg(updateQuery.lastError().text()));
    }
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"导入完成: %1 个数据包, 耗时 %2 毫秒").arg(packetCount).arg(duration));
...
void DatabaseManager::importToTempDb(ImportContext& ctx, QAtomicInt& processedCount, 
                                      ...
                                      int totalMessages)
{
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 开始导入临时库 - 文件ID: %1, 起始索引: %2, 结束索引: %3, 数据量: %4")
    ...
    
    {
        ...
        if (!tempDb.open()) {
            logError(tr(u8"无法打开临时数据库: %1").arg(tempDb.lastError().text()));
            LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 打开临时库失败: %1, 错误: %2")
                     .arg(ctx.tempDbPath).arg(tempDb.lastError().text()));
            QSqlDatabase::removeDatabase(connectionName);
            return;
        }
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 临时库已打开: %1").arg(ctx.tempDbPath));
        ...
        if (!tempDb.transaction()) {
            LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 开始临时库事务失败: %1").arg(tempDb.lastError().text()));
            tempDb.close();
            QSqlDatabase::removeDatabase(connectionName);
            return;
        }
        ...
        for (int i = 0; i < ctx.data.size(); ++i) {
            if (m_importCanceled.load()) {
                LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 临时库导入被取消 - 文件ID: %1, 已插入: %2")
            ...
            int actualMsgIndex = ctx.startIdx + i;
            QVector<DbDataRecord> msgRecords = convertToRecords(msg, ctx.fileId, actualMsgIndex, globalIndex);
            
            LOG_DEBUG("DatabaseManager", QString::fromUtf8(u8"[数据库] 消息 %1 - actualMsgIndex: %2, 数据包数: %3")
            ...
            if (progressCallback && (i % 100 == 0)) {
                ...
                progressCallback(cappedCurrent, totalMessages, tr(u8"正在导入数据..."));
            } else {
        ...
        if (canceled) {
            tempDb.rollback();
            LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 临时库事务已回滚 - 文件ID: %1").arg(ctx.fileId));
        } else {
            if (!records.isEmpty()) {
                batchInsertPackets(tempDb, ctx.fileId, records);
                insertColumnarData(tempDb, ctx.fileId, records);
            ...
            if (!tempDb.commit()) {
                LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 提交临时库失败: %1").arg(tempDb.lastError().text()));
            }
            
            LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 临时库导入完成 - 文件ID: %1, 插入记录数: %2")
                     .arg(ctx.fileId).arg(totalRecordsInserted));
        }
                                          ...
                                          int totalMessages)
{
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 新数据起始row_index: %1").arg(currentMaxRowIndex));
    
    for (int i = 0; i < tempDbs.size(); ++i) {
        const QString& tempDb = tempDbs[i];
        QString alias = QString("temp_%1").arg(i);
        
        LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并临时库: %1, 别名: %2").arg(tempDb).arg(alias));
        
        if (!QFile::exists(tempDb)) {
            LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 临时库文件不存在: %1").arg(tempDb));
            continue;
        }
        ...
        
        {
            ...
            if (!query.exec(QString("ATTACH DATABASE '%1' AS %2").arg(tempDb).arg(alias))) {
                LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] ATTACH临时库失败: %1").arg(query.lastError().text()));
                continue;
            }
            LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] ATTACH成功"));
            
            if (query.exec(QString("SELECT COUNT(*) FROM %1.packets").arg(alias)) && query.next()) {
                tempRowCount = query.value(0).toLongLong();
            }
            LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 临时库记录数: %1").arg(tempRowCount));
            
            if (tempRowCount == 0) {
                LOG_WARNING("DatabaseManager", QString::fromUtf8(u8"[数据库] 临时库无数据，跳过: %1").arg(tempDb));
                query.exec(QString("DETACH DATABASE %1").arg(alias));
                continue;
            }
            
            if (!db.transaction()) {
                LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 开始合并事务失败: %1").arg(db.lastError().text()));
                query.exec(QString("DETACH DATABASE %1").arg(alias));
                continue;
            }
            ...
            if (!query.exec(insertPacketsSql)) {
                LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并packets失败: %1").arg(query.lastError().text()));
                hasError = true;
            } else {
                LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并packets成功, 影响行数: %1").arg(query.numRowsAffected()));
            }
            ...
            if (!hasError) {
                ...
                if (!query.exec(insertColTerminalSql)) {
                    LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并col_terminal失败: %1").arg(query.lastError().text()));
                    hasError = true;
                } else {
                    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并col_terminal成功, 影响行数: %1").arg(query.numRowsAffected()));
                }
            }
            ...
            if (!hasError) {
                ...
                if (!query.exec(insertColTimestampSql)) {
                    LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并col_timestamp失败: %1").arg(query.lastError().text()));
                    hasError = true;
                } else {
                    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并col_timestamp成功, 影响行数: %1").arg(query.numRowsAffected()));
                }
            }
            ...
            if (!hasError) {
                ...
                if (!query.exec(insertColTypeSql)) {
                    LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并col_type失败: %1").arg(query.lastError().text()));
                    hasError = true;
                } else {
                    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并col_type成功, 影响行数: %1").arg(query.numRowsAffected()));
                }
            }
            ...
            if (!hasError) {
                ...
                if (!query.exec(insertColStatusSql)) {
                    LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并col_status失败: %1").arg(query.lastError().text()));
                    hasError = true;
                } else {
                    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并col_status成功, 影响行数: %1").arg(query.numRowsAffected()));
                }
            }
            ...
            if (hasError) {
                LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并过程中出现错误，回滚事务 (临时库 %1)").arg(i));
                db.rollback();
            } else {
                if (!db.commit()) {
                    LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并事务提交失败: %1").arg(db.lastError().text()));
                    db.rollback();
                } else {
                    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 合并事务已提交 (临时库 %1)").arg(i));
                    mergeSuccess = true;
                }
            }
        }
        
        {
            ...
            if (!detachQuery.exec(QString("DETACH DATABASE %1").arg(alias))) {
                LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] DETACH临时库失败: %1").arg(detachQuery.lastError().text()));
            } else {
                LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] DETACH成功: %1").arg(alias));
            }
        }
        ...
        if (progressCallback && totalMessages > 0) {
            ...
            progressCallback(baseProgress + mergeProgress, totalMessages, tr(u8"合并数据 (%1/%2)...").arg(i + 1).arg(tempDbs.size()));
        }
    }
}
...
void DatabaseManager::batchInsertPackets(QSqlDatabase& db, int fileId, const QVector<DbDataRecord>& records)
{
    ...
    if (!query.exec()) {
        logError(tr(u8"批量插入数据失败: %1").arg(query.lastError().text()));
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 批量插入失败 - 记录数: %1, 错误: %2, 连接: %3, 是否打开: %4")
...
void DatabaseManager::insertColumnarData(QSqlDatabase& db, int fileId, const QVector<DbDataRecord>& records)
{
    ...
    // 插入终端地址数据
    {
        ...
        if (!query.exec()) {
            logError(tr(u8"插入终端地址列式数据失败: %1").arg(query.lastError().text()));
        }
    }
    ...
    // 插入时间戳数据
    {
        ...
        if (!query.exec()) {
            logError(tr(u8"插入时间戳列式数据失败: %1").arg(query.lastError().text()));
        }
    }
    ...
    // 插入消息类型数据
    {
        ...
        if (!query.exec()) {
            logError(tr(u8"插入消息类型列式数据失败: %1").arg(query.lastError().text()));
        }
    }
}
    ...
    int errorFlagFilter)
{
    ...
    // 执行查询
    if (!query.exec()) {
        logError(tr(u8"查询数据包失败: %1").arg(query.lastError().text()));
        return records;
    }
...
QVector<DbDataRecord> DatabaseManager::queryPacketsByMsgIndex(int fileId, int msgIndex)
{
    ...
    if (!query.exec()) {
        logError(tr(u8"按msgIndex查询数据包失败: %1").arg(query.lastError().text()));
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[queryPacketsByMsgIndex] 查询失败, fileId: %1, msgIndex: %2, 错误: %3")
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[queryPacketsByMsgIndex] 查询完成, fileId: %1, msgIndex: %2, 返回记录数: %3")
...
MessageTypeDistribution DatabaseManager::queryMessageTypeDistribution(int fileId)
{
    QString cacheKey = QString("type_dist_%1").arg(fileId);
    QVariant cached = getCachedStatistics(fileId, cacheKey);
    
    if (cached.isValid()) {
        return cached.value<MessageTypeDistribution>();
    }
...
bool DatabaseManager::switchCurrentFile(int fileId)
{
    ...
    if (!query.exec()) {
        db.rollback();
        logError(tr(u8"切换文件失败: %1").arg(query.lastError().text()));
        return false;
    }
...
bool DatabaseManager::deleteFile(int fileId)
{
    ...
    if (!query.exec()) {
        db.rollback();
        logError(tr(u8"删除文件失败: %1").arg(query.lastError().text()));
        return false;
    }
...
{
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 删除文件数据, 文件ID: %1").arg(fileId));
    
    QSqlDatabase& db = getWriteConnection();
    
    if (!db.transaction()) {
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 开始删除事务失败: %1").arg(db.lastError().text()));
        return false;
    }
    ...
    query.addBindValue(fileId);
    if (!query.exec()) {
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 删除packets失败: %1").arg(query.lastError().text()));
    }
    
    query.prepare("DELETE FROM packets_col_terminal WHERE file_id = ?");
    query.addBindValue(fileId);
    if (!query.exec()) {
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 删除col_terminal失败: %1").arg(query.lastError().text()));
    }
    
    query.prepare("DELETE FROM packets_col_timestamp WHERE file_id = ?");
    query.addBindValue(fileId);
    if (!query.exec()) {
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 删除col_timestamp失败: %1").arg(query.lastError().text()));
    }
    
    query.prepare("DELETE FROM packets_col_type WHERE file_id = ?");
    query.addBindValue(fileId);
    if (!query.exec()) {
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 删除col_type失败: %1").arg(query.lastError().text()));
    }
    
    query.prepare("DELETE FROM packets_col_status WHERE file_id = ?");
    query.addBindValue(fileId);
    if (!query.exec()) {
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 删除col_status失败: %1").arg(query.lastError().text()));
    }
    
    query.prepare("DELETE FROM files WHERE id = ?");
    query.addBindValue(fileId);
    if (!query.exec()) {
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 删除文件记录失败: %1").arg(query.lastError().text()));
    }
    
    if (!db.commit()) {
        db.rollback();
        LOG_ERROR("DatabaseManager", QString::fromUtf8(u8"[数据库] 提交删除事务失败: %1").arg(db.lastError().text()));
        return false;
    }
    
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 成功删除文件数据, 文件ID: %1").arg(fileId));
    return true;
}
...
{
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 清除文件数据包, 文件ID: %1").arg(fileId));
    ...
    LOG_INFO("DatabaseManager", QString::fromUtf8(u8"[数据库] 已清除文件数据包, 文件ID: %1").arg(fileId));
    return true;
}
...
QVector<DbDataRecord> DatabaseManager::convertToRecords(const PacketParser::SMbiMonPacketMsg& msg, int fileId, int msgIndex, int& globalIndex)
{
    QVector<DbDataRecord> records;
    
    // 解析命令字
    auto parseCmd = [](quint16 cmdRaw, int& terminalAddr, int& subAddr, int& trBit, int& modeCode, int& wordCount) {
        PacketParser::CMD cmd;
        memcpy(&cmd, &cmdRaw, sizeof(PacketParser::CMD));
    ...
    for (int i = 0; i < msg.packetDatas.size(); ++i) {
        const PacketParser::SMbiMonPacketData& packetData = msg.packetDatas[i];
