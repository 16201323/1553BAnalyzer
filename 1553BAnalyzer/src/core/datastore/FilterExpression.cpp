...
{
    // 按分号分割多个条件组
    QStringList groupStrs = m_expression.split(';', QT5COMPAT_SKIP_EMPTY);
...
bool FilterExpression::parseConditionGroup(const QString& groupStr, ConditionGroup& group)
{
    ...
    while ((match = itemPattern.match(groupStr, pos)).hasMatch()) {
        ...
        if (!firstItem) {
            ...
            if (between == "&&") {
                ...
                m_error = QString::fromUtf8(u8"无效的逻辑运算符: %1").arg(between);
                return false;
            }
        }
    ...
    if (group.items.isEmpty()) {
        m_error = QString::fromUtf8(u8"条件组中未找到有效的条件: %1").arg(groupStr);
        return false;
    }
...
bool FilterExpression::parseConditionItem(const QString& itemStr, ConditionItem& item)
{
    ...
    if (!match.hasMatch()) {
        m_error = QString::fromUtf8(u8"无效的条件项: %1").arg(itemStr);
        return false;
    }
...
bool FilterExpression::parseCompareOperator(const QString& opStr, CompareOperator& op)
{
    if (opStr == ">") {
        ...
        m_error = QString::fromUtf8(u8"无效的比较运算符: %1").arg(opStr);
        return false;
    }
    return true;
}
