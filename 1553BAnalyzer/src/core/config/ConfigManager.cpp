...
ConfigManager::ConfigManager(QObject *parent)
    ...
    , m_retryCount(3)
{
    LOG_DEBUG("ConfigManager", u8"ConfigManager实例创建");
    loadDefaults();
}
...
{
    LOG_DEBUG("ConfigManager", u8"ConfigManager实例销毁");
}
...
{
    LOG_DEBUG("ConfigManager", u8"加载默认配置");
    ...
    LOG_DEBUG("ConfigManager", u8"默认配置加载完成");
}
...
{
    LOG_INFO("ConfigManager", QString::fromUtf8(u8"从资源文件复制配置到: %1").arg(targetPath));
    
    QFile resourceFile(":/config/config.xml");
    if (!resourceFile.exists()) {
        LOG_ERROR("ConfigManager", QString::fromUtf8(u8"资源文件中不存在config.xml"));
        m_lastError = tr(u8"资源文件中不存在默认配置");
        return false;
    }
    
    if (!resourceFile.open(QIODevice::ReadOnly)) {
        LOG_ERROR("ConfigManager", QString::fromUtf8(u8"无法打开资源文件中的config.xml"));
        m_lastError = tr(u8"无法打开资源文件中的默认配置");
        return false;
    }
    ...
    if (!dir.exists()) {
        if (!dir.mkpath(".")) {
            LOG_ERROR("ConfigManager", QString::fromUtf8(u8"无法创建配置目录: %1").arg(dir.absolutePath()));
            m_lastError = tr(u8"无法创建配置目录: %1").arg(dir.absolutePath());
            return false;
        }
        LOG_INFO("ConfigManager", QString::fromUtf8(u8"创建配置目录: %1").arg(dir.absolutePath()));
    }
    
    QFile targetFile(targetPath);
    if (!targetFile.open(QIODevice::WriteOnly)) {
        LOG_ERROR("ConfigManager", QString::fromUtf8(u8"无法创建配置文件: %1").arg(targetPath));
        m_lastError = tr(u8"无法创建配置文件: %1").arg(targetPath);
        return false;
    }
    ...
    LOG_INFO("ConfigManager", QString::fromUtf8(u8"配置文件复制成功: %1").arg(targetPath));
    return true;
}
...
bool ConfigManager::loadConfig(const QString& filePath)
{
    ...
    LOG_INFO("ConfigManager", QString::fromUtf8(u8"开始加载配置文件: %1").arg(configPath));
    
    QFile file(configPath);
    if (!file.exists()) {
        LOG_WARNING("ConfigManager", QString::fromUtf8(u8"配置文件不存在: %1").arg(configPath));
        
        if (!copyConfigFromResource(configPath)) {
            LOG_WARNING("ConfigManager", QString::fromUtf8(u8"从资源文件复制失败，使用默认配置"));
            loadDefaults();
    ...
    if (!parser.parse(configPath)) {
        ...
        LOG_ERROR("ConfigManager", QString::fromUtf8(u8"配置文件解析失败: %1").arg(m_lastError));
        loadDefaults();
        emit configLoadFailed(m_lastError);
        return false;
    }
    ...
    m_speechConfig = parser.speechConfig();
    
    m_configLoaded = true;
    
    LOG_INFO("ConfigManager", QString::fromUtf8(u8"配置文件加载成功，共加载 %1 个模型提供商")
...
bool ConfigManager::saveConfig(const QString& filePath)
{
    ...
    LOG_INFO("ConfigManager", QString::fromUtf8(u8"保存配置到文件: %1").arg(configPath));
    ...
    bool result = parser.save(configPath, m_parserConfig, m_ganttConfig, m_databaseConfig,
                       m_modelProviders, m_defaultModel, m_apiTimeout, m_retryCount, m_reportFormat, m_speechConfig);
    
    if (result) {
        LOG_INFO("ConfigManager", QString::fromUtf8(u8"配置保存成功"));
        // 通知所有模块配置已变更，即时生效
        emit configChanged();
    } else {
        LOG_ERROR("ConfigManager", QString::fromUtf8(u8"配置保存失败: %1").arg(parser.lastError()));
        m_lastError = parser.lastError();
    }
...
void ConfigManager::setParserConfig(const ParserConfig& config)
{
    ...
    LOG_DEBUG("ConfigManager", u8"解析器配置已更新");
}
...
void ConfigManager::setGanttConfig(const GanttConfig& config)
{
    ...
    LOG_DEBUG("ConfigManager", u8"甘特图配置已更新");
}
...
void ConfigManager::setDatabaseConfig(const DatabaseConfig& config)
{
    ...
    LOG_DEBUG("ConfigManager", QString::fromUtf8(u8"数据库配置已更新, 阈值: %1").arg(config.recordThreshold));
}
...
void ConfigManager::setModelProviders(const QVector<ModelProvider>& providers)
{
    ...
    LOG_DEBUG("ConfigManager", QString::fromUtf8(u8"模型提供商列表已更新，共 %1 个").arg(providers.size()));
}
...
void ConfigManager::setDefaultModel(const QString& model)
{
    ...
    LOG_DEBUG("ConfigManager", QString::fromUtf8(u8"默认模型已设置为: %1").arg(model));
}

void ConfigManager::setApiTimeout(int timeout)
{
    m_apiTimeout = timeout;
    LOG_DEBUG("ConfigManager", QString::fromUtf8(u8"API超时已设置为: %1秒").arg(timeout));
}

void ConfigManager::setRetryCount(int count)
{
    m_retryCount = count;
    LOG_DEBUG("ConfigManager", QString::fromUtf8(u8"重试次数已设置为: %1").arg(count));
}
...
void ConfigManager::setReportFormat(const QString& format)
{
    ...
    LOG_DEBUG("ConfigManager", QString::fromUtf8(u8"报告格式已设置为: %1").arg(format));
}
 ...
 * @return 语音识别配置结构体
 */
SpeechConfig ConfigManager::getSpeechConfig() const
{
...
/**
 * @brief 设置语音识别配置
 * @param config 语音识别配置结构体
 */
void ConfigManager::setSpeechConfig(const SpeechConfig& config)
{
    m_speechConfig = config;
    LOG_DEBUG("ConfigManager", QString::fromUtf8(u8"语音配置已更新，模型: %1, 启用: %2")
              .arg(config.engine.modelPath)
              .arg(config.engine.enabled ? "true" : "false"));
}
...
QString ConfigManager::getConfigFilePath() const
{
    ...
    if (QFile::exists(appConfigPath)) {
        LOG_INFO("ConfigManager", QString::fromUtf8(u8"使用exe目录配置文件: %1").arg(appConfigPath));
    ...
    if (!canonicalPath.isEmpty() && QFile::exists(canonicalPath)) {
        LOG_INFO("ConfigManager", QString::fromUtf8(u8"使用源码目录配置文件: %1").arg(canonicalPath));
        return canonicalPath;
    }
    
    // 都不存在时返回默认路径（后续会从资源文件复制）
    LOG_INFO("ConfigManager", QString::fromUtf8(u8"配置文件未找到，将使用默认路径: %1").arg(appConfigPath));
    return appConfigPath;
}
