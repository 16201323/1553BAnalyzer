...
void ModelAdapter::sendOpenAIRequest(const QString& query, const QJsonObject& context)
{
    ...
    emit progressUpdate(tr(u8"正在发送请求..."));
}
...
{
    ModelResponse response;
    /* success 和 tokensUsed 已有默认值 false 和 0 */
    ...
    if (result.contains("error")) {
        ...
        if (errorObj.contains("message")) {
            response.error = errorObj["message"].toString();
        } else {
            response.error = tr(u8"API返回错误，但未提供错误信息");
        }
        return response;
    }
    
    /* 解析OpenAI格式的成功响应：choices[0].message.content */
    if (result.contains("choices")) {
        ...
        response.error = tr(u8"API响应缺少choices字段");
    }
