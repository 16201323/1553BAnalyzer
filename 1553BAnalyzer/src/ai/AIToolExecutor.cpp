...
{
    LOG_INFO("AIToolExecutor", QString::fromUtf8(u8"执行工具: %1").arg(toolName));
    ...
    } else if (toolName == "analyze_time_interval") {
        result = executeAnalyzeTimeInterval(arguments);
    } else {
        result.success = false;
        result.message = tr(u8"未知的工具: %1").arg(toolName);
    }
...
ToolResult AIToolExecutor::executeGetAnomalies(const QJsonObject& args)
{
    ...
    QString anomalyDesc;
    if (anomalyType == "communication_failure") anomalyDesc = tr(u8"通信失败");
    else if (anomalyType == "busy_flag") anomalyDesc = tr(u8"忙标志位(Busy)");
    else if (anomalyType == "subsystem_fault") anomalyDesc = tr(u8"子系统故障");
    else if (anomalyType == "service_request") anomalyDesc = tr(u8"服务请求");
    else if (anomalyType == "timeout") anomalyDesc = tr(u8"超时错误");
    else anomalyDesc = tr(u8"全部异常");
    
    if (filteredCount > 0) {
        result.message = tr(u8"检测到 %1 条%2数据（占总数 %3%）")
            .arg(filteredCount)
            .arg(anomalyDesc)
            .arg(totalCount > 0 ? QString::number(filteredCount * 100.0 / totalCount, 'f', 1) : "0");
    } else {
        result.message = tr(u8"未检测到%1数据").arg(anomalyDesc);
    }
...
ToolResult AIToolExecutor::executeGetTopN(const QJsonObject& args)
{
    ...
    if (subject == "active_terminal") {
        ...
        QString msg = tr(u8"最频繁的子地址（前%1名）：\n").arg(topN);
        for (int i = 0; i < qMin(topN, sortedList.size()); ++i) {
            msg += tr(u8"  %1. 子地址 %2：%3 条消息\n").arg(i + 1).arg(sortedList[i].first).arg(sortedList[i].second);
        }
        result.message = msg;
        ...
        QString msg = tr(u8"错误最多的终端（前%1名）：\n").arg(topN);
        for (int i = 0; i < qMin(topN, sortedList.size()); ++i) {
            msg += tr(u8"  %1. 终端 %2：%3 条错误\n").arg(i + 1).arg(sortedList[i].first).arg(sortedList[i].second);
        }
        result.message = msg;
        ...
        auto typeToString = [](MessageType type) -> QString {
            switch (type) {
            case MessageType::BC_TO_RT: return QString::fromUtf8(u8"BC→RT");
            case MessageType::RT_TO_BC: return QString::fromUtf8(u8"RT→BC");
            case MessageType::RT_TO_RT: return QString::fromUtf8(u8"RT→RT");
            case MessageType::Broadcast: return QString::fromUtf8(u8"广播");
            default: return QString::fromUtf8(u8"未知");
            }
        };
        
        QString msg = tr(u8"消息类型频率排名：\n");
        for (int i = 0; i < qMin(topN, sortedList.size()); ++i) {
        ...
        result.success = false;
        result.message = tr(u8"未知的排名主题：%1").arg(subject);
    }
...
ToolResult AIToolExecutor::executeGenerateReport(const QJsonObject& args)
{
    ...
    result.success = true;
    result.message = tr(u8"正在生成%1格式智能分析报告...").arg(format.toUpper());
...
ToolResult AIToolExecutor::executeAddFilter(const QJsonObject& args)
{
    ...
    result.success = true;
    result.message = tr(u8"已追加筛选条件，当前显示 %1 条数据").arg(count);
    result.data = filters;
...
ToolResult AIToolExecutor::executeApplyFiltersBatch(const QJsonObject& args)
{
    ...
    int count = m_dataStore->filteredCount();
    result.success = true;
    result.message = tr(u8"已批量设置筛选条件，当前显示 %1 条数据").arg(count);
    result.data = filters;
...
ToolResult AIToolExecutor::executePreviewFilter(const QJsonObject& args)
{
    ...
    if (previewCount >= 0) {
        ...
        result.message = tr(u8"预览：满足条件的数据约 %1 条（占总数 %2%，共 %3 条）")
            .arg(previewCount)
        ...
        result.success = true;
        result.message = tr(u8"预览功能仅支持数据库模式，当前为内存模式");
    }
...
ToolResult AIToolExecutor::executeQueryData(const QJsonObject& args)
{
    ...
    int count = m_dataStore->filteredCount();
    result.success = true;
    result.message = tr(u8"已应用筛选条件，当前显示 %1 条数据").arg(count);
    result.data = filters;
...
ToolResult AIToolExecutor::executeGenerateChart(const QJsonObject& args)
{
    ...
    QString subjectName;
    if (subject == "message_type") subjectName = tr(u8"消息类型分布");
    else if (subject == "terminal") subjectName = tr(u8"终端数据量");
    else if (subject == "chstt") subjectName = tr(u8"收发状态");
    else if (subject == "time") subjectName = tr(u8"时间趋势");
    else subjectName = subject;
    
    QString chartTypeName;
    if (chartType == "pie") chartTypeName = tr(u8"饼图");
    else if (chartType == "bar") chartTypeName = tr(u8"柱状图");
    else if (chartType == "line") chartTypeName = tr(u8"折线图");
    else chartTypeName = chartType;
    
    result.success = true;
    result.message = tr(u8"已生成%1：%2").arg(chartTypeName, subjectName);
...
ToolResult AIToolExecutor::executeClearFilter(const QJsonObject& args)
{
    ...
    result.success = true;
    result.message = tr(u8"已清除所有筛选条件，显示全部数据");
 ...
 * @brief 执行时间间隔分析工具
 * 
 * 分析指定RT和子地址的时间间隔，生成折线图展示时间间隔变化趋势
 */
ToolResult AIToolExecutor::executeAnalyzeTimeInterval(const QJsonObject& args)
{
    ToolResult result;
    result.actionType = "time_interval_analysis";
    ...
    // 获取必填参数
    if (!args.contains("terminal_address") || !args.contains("sub_address")) {
        result.success = false;
        result.message = tr(u8"缺少必填参数：terminal_address 或 sub_address");
        return result;
    }
    
    int terminalAddress = args["terminal_address"].toInt();
    int subAddress = args["sub_address"].toInt();
    
    // 参数范围校验
    if (terminalAddress < 0 || terminalAddress > 31) {
        result.success = false;
        result.message = tr(u8"终端地址无效，应在0-31范围内");
        return result;
    }
    
    if (subAddress < 0 || subAddress > 31) {
        result.success = false;
        result.message = tr(u8"子地址无效，应在0-31范围内");
        return result;
    }
    ...
    if (args.contains("time_range")) {
        ...
        if (timeRange.contains("start")) {
            ...
            if (parts.size() >= 3) {
                ...
                int s = secParts[0].toInt();
                int ms = secParts.size() > 1 ? secParts[1].left(3).toInt() : 0;
                startTime = static_cast<qint64>(h) * 3600000 + m * 60000 + s * 1000 + ms;
            }
        }
        ...
        if (timeRange.contains("end")) {
            ...
            if (parts.size() >= 3) {
                ...
                int s = secParts[0].toInt();
                int ms = secParts.size() > 1 ? secParts[1].left(3).toInt() : 0;
                endTime = static_cast<qint64>(h) * 3600000 + m * 60000 + s * 1000 + ms;
            }
        }
    }
    
    // 是否导出CSV
    bool exportCsv = args.contains("export_csv") ? args["export_csv"].toBool() : false;
    
    // 执行时间间隔分析
    TimeIntervalAnalysis analysis = TimeIntervalAnalyzer::analyze(
        m_dataStore, terminalAddress, subAddress, startTime, endTime);
    
    // 构建返回结果
    result.success = (analysis.recordCount >= 2);
    
    if (analysis.recordCount < 2) {
        result.message = tr(u8"RT%1 子地址%2 的数据不足（仅%3条记录，需要至少2条）")
            .arg(terminalAddress)
            .arg(subAddress)
            .arg(analysis.recordCount);
    } else {
        // 构建统计信息消息
        QString stabilityIcon = analysis.isStable ? QString::fromUtf8(u8"✓") : QString::fromUtf8(u8"✗");
        
        result.message = tr(u8"RT%1 子地址%2 时间间隔分析结果：\n"
            u8"  记录数：%3 条\n"
            u8"  平均间隔：%4 ms\n"
            u8"  最小间隔：%5 ms\n"
            u8"  最大间隔：%6 ms\n"
            ...
            .arg(analysis.minIntervalMs, 0, 'f', 2)
            .arg(analysis.maxIntervalMs, 0, 'f', 2)
        ...
        analysisData["terminal_address"] = terminalAddress;
        analysisData["sub_address"] = subAddress;
        analysisData["record_count"] = analysis.recordCount;
        analysisData["avg_interval_ms"] = analysis.avgIntervalMs;
        analysisData["min_interval_ms"] = analysis.minIntervalMs;
        analysisData["max_interval_ms"] = analysis.maxIntervalMs;
        analysisData["std_dev_ms"] = analysis.stdDevMs;
        analysisData["jitter_percent"] = analysis.jitterPercent;
        analysisData["is_stable"] = analysis.isStable;
        analysisData["stability_assessment"] = analysis.stabilityAssessment;
        
        // 时间戳和间隔数据（用于图表）
        QJsonArray timestampsArray;
        QJsonArray intervalsArray;
        for (int i = 0; i < analysis.intervals.size(); ++i) {
            timestampsArray.append(analysis.timestamps[i + 1]);  // 从第二条记录开始
            intervalsArray.append(analysis.intervals[i]);
        }
        analysisData["timestamps"] = timestampsArray;
        analysisData["intervals"] = intervalsArray;
        
        result.data = analysisData;
        
        // 如果需要导出CSV
        if (exportCsv) {
            QString defaultPath = QString(u8"RT%1_SA%2_时间间隔分析.csv")
                .arg(terminalAddress).arg(subAddress);
            QString csvPath = QFileDialog::getSaveFileName(
                nullptr,
                tr(u8"导出时间间隔数据"),
                defaultPath,
                tr(u8"CSV文件;;All Files (*)"));
            
            if (!csvPath.isEmpty()) {
                if (TimeIntervalAnalyzer::exportToCsv(analysis, csvPath)) {
                    result.message += tr(u8"\n\nCSV文件已导出：%1").arg(csvPath);
                } else {
                    result.message += tr(u8"\n\nCSV导出失败");
                }
            }
    ...
    // 发送信号通知界面更新
    emit timeIntervalAnalysisRequested(analysis);
