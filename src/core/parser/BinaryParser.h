 ...
 *     QVector<SMbiMonPacketMsg> data = parser.getParsedData();
 *     // 处理解析后的数据
 * }
 * @endcode
 * 
...
public:
     ...
     */
    Q_INVOKABLE QVector<SMbiMonPacketMsg> getParsedData() const;
...
private:
     ...
     * - timestamp: 4字节，时间戳
     */
    bool parseHeader(QDataStream& stream, SMbiMonPacketHeader& header);
     ...
     * - datas: 变长，数据内容
     */
    bool parsePacketData(QDataStream& stream, SMbiMonPacketData& data, int remainingLen, int& bytesRead);
    ...
    QVector<SMbiMonPacketMsg> m_parsedData;  ///< 解析后的消息数组
    QString m_lastError;           ///< 最后的错误信息
};

Q_DECLARE_METATYPE(QVector<SMbiMonPacketMsg>)
