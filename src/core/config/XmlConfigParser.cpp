    ...
    , m_reportFormat("html")
{
    ...
    LOG_DEBUG("XmlConfigParser", "XmlConfigParser实例创建");
}
...
{
    LOG_INFO("XmlConfigParser", QString("开始解析配置文件: %1").arg(filePath));
    
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly)) {
        m_lastError = QCoreApplication::translate("XmlConfigParser", "无法打开配置文件: %1").arg(filePath);
        LOG_ERROR("XmlConfigParser", m_lastError);
        return false;
    }
    
    LOG_DEBUG("XmlConfigParser", "配置文件打开成功，开始读取XML内容");
    ...
    while (!xml.atEnd() && !xml.hasError()) {
        ...
        if (token == QXmlStreamReader::StartElement) {
            ...
            LOG_DEBUG("XmlConfigParser", QString("发现XML元素: %1").arg(elementName));
            
            if (elementName == "Parser") {
                if (!parseParserSection(xml)) {
                    LOG_WARNING("XmlConfigParser", "解析Parser节点失败");
                }
            } else if (elementName == "GanttChart") {
                if (!parseGanttSection(xml)) {
                    LOG_WARNING("XmlConfigParser", "解析GanttChart节点失败");
                }
            } else if (elementName == "Database") {
                if (!parseDatabaseSection(xml)) {
                    LOG_WARNING("XmlConfigParser", "解析Database节点失败");
                }
            } else if (elementName == "Models") {
                if (!parseModelsSection(xml)) {
                    LOG_WARNING("XmlConfigParser", "解析Models节点失败");
                }
            } else if (elementName == "Export") {
                if (!parseExportSection(xml)) {
                    LOG_WARNING("XmlConfigParser", "解析Export节点失败");
                }
            }
        }
    }
    ...
    if (xml.hasError()) {
        m_lastError = QCoreApplication::translate("XmlConfigParser", "XML解析错误: %1").arg(xml.errorString());
        LOG_ERROR("XmlConfigParser", QString("XML解析错误: %1, 行号: %2")
                  .arg(xml.errorString())
                  .arg(xml.lineNumber()));
        file.close();
        return false;
    ...
    LOG_INFO("XmlConfigParser", QString("配置文件解析完成，共加载 %1 个模型提供商")
...
{
    LOG_DEBUG("XmlConfigParser", "开始解析Parser配置节点");
    ...
    while (!(xml.tokenType() == QXmlStreamReader::EndElement && 
        ...
        if (xml.tokenType() == QXmlStreamReader::StartElement) {
            ...
            LOG_DEBUG("XmlConfigParser", QString("解析配置项: %1 = %2").arg(name, text));
    ...
    LOG_INFO("XmlConfigParser", QString("Parser配置解析完成，共解析 %1 个配置项").arg(parsedCount));
    return true;
}
...
{
    LOG_DEBUG("XmlConfigParser", "开始解析GanttChart配置节点");
    ...
    while (!(xml.tokenType() == QXmlStreamReader::EndElement && 
        ...
        if (xml.tokenType() == QXmlStreamReader::StartElement) {
            ...
            LOG_DEBUG("XmlConfigParser", QString("解析颜色配置: %1 = %2").arg(name, text));
    ...
    LOG_INFO("XmlConfigParser", QString("GanttChart配置解析完成，共解析 %1 个颜色配置").arg(parsedCount));
    return true;
}
...
{
    LOG_DEBUG("XmlConfigParser", "开始解析Database配置节点");
    ...
    while (!(xml.tokenType() == QXmlStreamReader::EndElement && 
        ...
        if (xml.tokenType() == QXmlStreamReader::StartElement) {
            ...
            LOG_DEBUG("XmlConfigParser", QString("解析数据库配置: %1 = %2").arg(name, text));
    ...
    LOG_INFO("XmlConfigParser", QString("Database配置解析完成，记录阈值: %1").arg(m_databaseConfig.recordThreshold));
    return true;
}
...
{
    LOG_DEBUG("XmlConfigParser", "开始解析Models配置节点");
    ...
    while (!(xml.tokenType() == QXmlStreamReader::EndElement && 
        ...
        if (xml.tokenType() == QXmlStreamReader::StartElement) {
            ...
            if (name == "DefaultModel") {
                QString text = xml.readElementText();
                m_defaultModel = text;
                LOG_DEBUG("XmlConfigParser", QString("默认模型: %1").arg(m_defaultModel));
            } else if (name == "ApiTimeout") {
                QString text = xml.readElementText();
                m_apiTimeout = text.toInt();
                LOG_DEBUG("XmlConfigParser", QString("API超时: %1秒").arg(m_apiTimeout));
            } else if (name == "RetryCount") {
                QString text = xml.readElementText();
                m_retryCount = text.toInt();
                LOG_DEBUG("XmlConfigParser", QString("重试次数: %1").arg(m_retryCount));
            } else if (name == "Provider") {
                ModelProvider provider;
                if (parseProvider(xml, provider)) {
                    m_modelProviders.append(provider);
                    providerCount++;
                    LOG_INFO("XmlConfigParser", QString("加载提供商: %1 (%2), 实例数: %3")
                              .arg(provider.name)
                              .arg(provider.id)
    ...
    LOG_INFO("XmlConfigParser", QString("Models配置解析完成，共加载 %1 个提供商").arg(providerCount));
    return true;
}

bool XmlConfigParser::parseExportSection(QXmlStreamReader& xml)
{
    LOG_DEBUG("XmlConfigParser", "开始解析Export配置节点");
    ...
    while (!(xml.tokenType() == QXmlStreamReader::EndElement && 
        ...
        if (xml.tokenType() == QXmlStreamReader::StartElement) {
            ...
            if (name == "ReportFormat") {
                ...
                LOG_DEBUG("XmlConfigParser", QString("报告格式: %1").arg(m_reportFormat));
            }
        }
    }
    
    LOG_INFO("XmlConfigParser", QString("Export配置解析完成，报告格式: %1").arg(m_reportFormat));
    return true;
}
...
bool XmlConfigParser::parseProvider(QXmlStreamReader& xml, ModelProvider& provider)
{
    ...
    LOG_DEBUG("XmlConfigParser", QString("解析Provider: id=%1, name=%2").arg(provider.id, provider.name));
        ...
        if (xml.tokenType() == QXmlStreamReader::StartElement) {
            ...
            if (name == "Instance") {
                ...
                if (parseInstance(xml, instance)) {
                    ...
                    instanceCount++;
                    LOG_DEBUG("XmlConfigParser", QString("加载模型实例: %1 (enabled=%2)")
                              .arg(instance.name)
    ...
    LOG_DEBUG("XmlConfigParser", QString("Provider解析完成，实例数: %1").arg(instanceCount));
    return true;
}
...
bool XmlConfigParser::parseInstance(QXmlStreamReader& xml, ModelInstance& instance)
{
    ...
    LOG_DEBUG("XmlConfigParser", QString("解析Instance: name=%1, enabled=%2")
              .arg(instance.name)
              .arg(instance.enabled ? "true" : "false"));
    ...
    while (!(xml.tokenType() == QXmlStreamReader::EndElement && 
        ...
        if (xml.tokenType() == QXmlStreamReader::StartElement) {
            ...
            if (name == "ApiKey") {
                ...
                LOG_DEBUG("XmlConfigParser", QString("ApiKey已设置，长度: %1").arg(text.length()));
            } else if (name == "ApiUrl") {
                ...
                LOG_DEBUG("XmlConfigParser", QString("SystemPrompt长度: %1").arg(text.length()));
            }
        }
    }
...
{
    LOG_INFO("XmlConfigParser", QString("开始保存配置文件: %1").arg(filePath));
    
    QFile file(filePath);
    if (!file.open(QIODevice::WriteOnly)) {
        m_lastError = QCoreApplication::translate("XmlConfigParser", "无法写入配置文件: %1").arg(filePath);
        LOG_ERROR("XmlConfigParser", m_lastError);
        return false;
    }
    ...
    LOG_INFO("XmlConfigParser", QString("配置文件保存成功，共保存 %1 个提供商").arg(providers.size()));
