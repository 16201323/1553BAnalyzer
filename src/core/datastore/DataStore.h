...
public:
     ...
     * @param data 原始消息数据数组
     */
    void setData(const QVector<SMbiMonPacketMsg>& data);
     ...
     * @return 原始消息的常引用
     */
    const SMbiMonPacketMsg& getMessage(int msgIndex) const;
     ...
     * @param index 预构建的索引
     */
    void setIndexedData(const QVector<SMbiMonPacketMsg>& rawData, const QVector<DataRecord>& index);
...
private:
    ...
    QVector<SMbiMonPacketMsg> m_rawData;      ///< 原始消息数据数组
    ...
    mutable SMbiMonPacketMsg m_cachedMessage; ///< 数据库模式下缓存的消息（用于getMessage）
