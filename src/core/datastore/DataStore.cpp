...
void DataStore::setStorageMode(StorageMode mode)
{
    if (m_storageMode == mode) {
        LOG_INFO("DataStore", QString("[数据存储] 已处于该模式: %1").arg(static_cast<int>(mode)));
        return;
    }
    
    LOG_INFO("DataStore", QString("[数据存储] 切换模式 - 从: %1 到: %2")
    ...
    if (m_useDatabase) {
        ...
        LOG_INFO("DataStore", QString("[数据存储] 数据库初始化结果: %1").arg(ok));
    }
}
...
{
    LOG_INFO("DataStore", QString("[数据存储] 开始导入数据, 使用数据库: %1, 数据量: %2, 文件路径: %3")
             .arg(m_useDatabase).arg(data.size()).arg(filePath));
    
    if (!m_useDatabase) {
        LOG_INFO("DataStore", "[数据存储] 非数据库模式, 使用内存模式");
        setData(data);
        return 0;
    }
    
    LOG_INFO("DataStore", "[数据存储] 调用数据库管理器并行导入...");
    int fileId = DatabaseManager::instance()->importDataParallel(data, filePath, progressCallback);
    LOG_INFO("DataStore", QString("[数据存储] 并行导入返回文件ID: %1").arg(fileId));
    
    if (fileId > 0) {
        m_currentFileId = fileId;
        // 数据库模式下不保留m_rawData，减少内存占用
        // 需要时通过getMessage()从数据库按需查询重建
        m_rawData.clear();
        m_allRecords.clear();
        m_filteredRecords.clear();
        m_filteredCountForDb = DatabaseManager::instance()->queryPacketCount(fileId);
        m_currentPage = 0;
        LOG_INFO("DataStore", QString("[数据存储] 导入后 - 筛选计数: %1, 总页数: %2")
                 .arg(m_filteredCountForDb).arg(totalPages()));
        updateCurrentPageCache();
        LOG_INFO("DataStore", QString("[数据存储] 当前页记录数: %1").arg(m_currentPageRecords.size()));
        emit dataChanged();
        emit pageChanged(m_currentPage, totalPages(), static_cast<int>(m_filteredCountForDb));
    } else {
        LOG_ERROR("DataStore", QString("[数据存储] 并行导入失败, 文件ID: %1, 数据库错误: %2")
 ...
 * @param data 原始消息数据数组
 */
void DataStore::setData(const QVector<SMbiMonPacketMsg>& data)
{
    clear();
 ...
 */
const SMbiMonPacketMsg& DataStore::getMessage(int msgIndex) const
{
    static SMbiMonPacketMsg emptyMsg;
    ...
    if (m_useDatabase && m_currentFileId > 0 && msgIndex >= 0) {
        ...
        if (dbRecords.isEmpty()) {
            LOG_WARNING("DataStore", QString("[getMessage] 数据库查询无结果, fileId: %1, msgIndex: %2").arg(m_currentFileId).arg(msgIndex));
            return emptyMsg;
        }
        
        // 重建SMbiMonPacketMsg结构
        m_cachedMessage = SMbiMonPacketMsg();
        ...
        LOG_INFO("DataStore", QString("[getMessage] 从数据库重建消息, fileId: %1, msgIndex: %2, 数据包数: %3")
            .arg(m_currentFileId).arg(msgIndex).arg(m_cachedMessage.packetDatas.size()));
        
        return m_cachedMessage;
    }
    
    LOG_WARNING("DataStore", QString("[getMessage] 无法获取消息, msgIndex: %1, useDatabase: %2, fileId: %3, rawDataSize: %4")
...
QVector<DataRecord> DataStore::getAllRecordsForReport() const
{
    if (m_useDatabase && m_currentFileId > 0) {
        ...
        LOG_INFO("DataStore", QString("[报告数据] 数据库模式查询全量数据, 记录数: %1, m_rawData.size: %2")
        ...
        LOG_INFO("DataStore", QString("[报告数据] chstt采样(前%1条): [%2]")
    ...
    LOG_INFO("DataStore", QString("[报告数据] 内存模式返回全量数据, 记录数: %1").arg(m_allRecords.size()));
    return m_allRecords;
}
...
)
{
    ...
    for (int msgIdx = 0; msgIdx < data.size(); ++msgIdx) {
        const SMbiMonPacketMsg& msg = data[msgIdx];
 ...
 * @param index 预构建的索引
 */
void DataStore::setIndexedData(const QVector<SMbiMonPacketMsg>& rawData, const QVector<DataRecord>& index)
{
    // 阻塞信号，避免多次触发UI更新
    blockSignals(true);
...
void DataStore::updateCurrentPageCache()
{
    if (m_useDatabase && m_currentFileId > 0) {
        ...
        LOG_INFO("DataStore", QString("[数据存储] 查询返回记录数: %1, m_rawData.size: %2, 当前页: %3, 每页大小: %4, 偏移: %5")
        ...
        for (const DbDataRecord& dbRecord : dbRecords) {
            ...
            if (!m_rawData.isEmpty() && dbRecord.msgIndex >= 0 && dbRecord.msgIndex < m_rawData.size()) {
                ...
                if (dbRecord.dataIndex >= 0 && dbRecord.dataIndex < msg.packetDatas.size()) {
                    ...
                    LOG_ERROR("DataStore", QString("[数据存储] dataIndex越界 - msgIndex: %1, dataIndex: %2, packetDatas.size: %3")
