...
void TableView::setupUI()
{
    ...
    m_actionShowDetail = m_contextMenu->addAction(tr("数据详细"));
    m_contextMenu->addSeparator();
    m_actionCopy = m_contextMenu->addAction(tr("复制选中行"));
    m_actionExportCSV = m_contextMenu->addAction(tr("导出为CSV"));
    m_actionExportExcel = m_contextMenu->addAction(tr("导出为Excel"));
    m_contextMenu->addSeparator();
    m_actionFilter = m_contextMenu->addAction(tr("按此值筛选"));
}
...
void TableView::exportSelection(const QString& format)
{
    QString filePath = QFileDialog::getSaveFileName(
        ...
        tr("导出数据"),
        QString(),
        format == "csv" ? tr("CSV文件 (*.csv)") : tr("Excel文件 (*.xlsx)")
    );
...
void TableView::onFilterByValue()
{
    ...
    QString expression = QInputDialog::getText(
        ...
        tr("当前值: %1\n\n"
           ...
           "  <3&&>1      小于3且大于1\n"
           "  <3||>10     小于3或大于10\n"
           "  >1;<10      大于1或小于10").arg(currentValue),
        QLineEdit::Normal,
        QString(),
        &ok
    );
