...
void GanttView::updateView()
{
    ...
    (void)QtConcurrent::run([this, store, scope, hasTimeRange, timeStart, timeEnd, zoomLevel]() {
        ...
        for (int idx = 0; idx < scopedRecords.size(); idx++) {
            ...
            itemData.tooltip = QString("时间: %1 ms\n类型: %2\n终端: %3\n行号: %4")
        ...
        QMetaObject::invokeMethod(this, [this, minTime, maxTime, pixelsPerUnit, sceneWidth,
            ...
            if (needsSampling) {
                ...
                QGraphicsTextItem* info = m_scene->addText(
                    QString("采样显示 %1/%2 条数据 (每%3条取1条)").arg(itemCount).arg(dataCount).arg(step),
                    infoFont);
                info->setDefaultTextColor(QColor(150, 150, 150));
                info->setPos(0, sceneHeight - 20);
            }
            
            LOG_INFO("GanttView", QString("创建的甘特图条块数: %1, 跳过数: %2").arg(itemCount).arg(skippedCount));
            LOG_INFO("GanttView", "========== 甘特图更新完成 ==========");
