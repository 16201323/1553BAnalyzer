...
void FileListPanel::setupUI()
{
    ...
    toolbar->addAction(tr("移除"), this, &FileListPanel::onRemoveSelected);
    toolbar->addAction(tr("清空"), this, &FileListPanel::clearFiles);
                               ...
                               qint64 messageCount, const QString& importTime)
{
    ...
    m_files.append(filePath);
    QString displayText = QString("%1 (%2条)").arg(fileName).arg(messageCount);
...
void FileListPanel::updateFileId(const QString& filePath, int fileId)
{
    for (int i = 0; i < m_listWidget->count(); ++i) {
        ...
        if (item->data(Qt::UserRole).toString() == filePath) {
            ...
            QString displayText = QString("%1 (%2条)").arg(fi.fileName()).arg(msgCount);
...
void FileListPanel::clearFiles()
{
    ...
    QMessageBox::StandardButton reply = QMessageBox::question(
        this, tr("确认清空"),
        tr("确定要清空所有 %1 个文件吗？此操作不可撤销。").arg(m_listWidget->count()),
...
void FileListPanel::onRemoveSelected()
{
    ...
    QMessageBox::StandardButton reply = QMessageBox::question(
        this, tr("确认移除"),
        tr("确定要移除选中的 %1 个文件吗？").arg(selected.size()),
        QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
